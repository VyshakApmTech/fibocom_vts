...
#ifdef CONFIG_8850CM_TARGET
    /***************CORE TASK Eg:ATEngin/LTE/IMS/AUDIO/Timer**************
 ****************************** NOTICE!!! ****************************
 Customers are not allowed to use priority more than OSI_PRIORITY_HIGH
 *********************************************************************/
    OSI_HIGHER_PRIORITY_MODEM_BASE = 47,
    OSI_HIGHER_PRIORITY_NORMAL = 50,
    OSI_HIGHER_PRIORITY_NORMAL_1 = 52,
    OSI_HIGHER_PRIORITY_NORMAL_2 = 56,
    OSI_HIGHEST_PRIORITY = 60,
    OSI_PRIORITY_HISR = 64, // reserved
#else
...
#ifdef CONFIG_FIBOCOM_LOW_VOLTAGE_NORMAL_POWEROFF
    OSI_SHUTDOWN_LOW_VOLTAGE_POWER_OFF = 0x4f47,    ///< 'LOF'low voltage poweroff
#endif
#ifdef CONFIG_FIBOCOM_BASE
    OSI_SHUTDOWN_NORMAL_POWER_OFF = 0x0001, ///< 'NOF' normal power off
    OSI_SHUTDOWN_PSM_POWER_OFF = 0x0002, ///< 'NOF' normal power off,and the shutdown is not executed
#endif
 ...
 *
 * \param irqn      IRQ number
 * \param cfg     GIC configuration
 * \return
 *      - true on success
 *      - false on invalid parameters
 */
bool osiIrqSetConfig(uint32_t irqn, uint32_t cfg);
...
/**
 * create a thread for Unisoc BSP.
 * Usage: follow osiThreadCreate. Task priority can be set maximum.
 */
osiThread_t *osiThreadCreateForCore(const char *name, osiCallback_t func, void *argument,
                                    uint32_t priority, uint32_t stack_size,
                                    uint32_t event_count);
...
#ifdef CONFIG_FIBOCOM_BASE
bool osiThreadCallback1(osiThread_t *thread, osiCallback_t cb, void *cb_ctx);
#endif
...
#ifdef CONFIG_FIBOCOM_BASE
// type {1: suspend; 2: 32k sleep}
typedef struct
{
    void (*sleep)(void *ctx, uint32_t type);
    void (*wakeup)(void *ctx, uint32_t type, uint32_t source);
} osiPmSleepCb_t;

/**
 * \breif set pm sleep callback
 *
 * \param sleep_cb  sleep callback, caller keep the memory
 * \param context
 */
void osiSetPmSleepCallback(osiPmSleepCb_t *sleep_cb, void *ctx);
bool osiMessageQueueTryPutToFront(osiMessageQueue_t *mq, const void *msg, uint32_t timeout);
void osiMessageQueueReset(osiMessageQueue_t *mq);
