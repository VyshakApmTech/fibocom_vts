...
extern osiMemPool_t *gOsiUserHeap;
#ifdef CONFIG_KERNEL_MULTIPLE_HEAP_SUPPORT
#define USER_HEAP_SIZE 0x800000
#endif
#endif
...
#ifdef CONFIG_FIBOCOM_BASE  // SPCSS01514332
#ifdef CONFIG_KERNEL_MULTIPLE_HEAP_SUPPORT
    heap_size = heap_size - USER_HEAP_SIZE;
#endif
#endif
...
#ifdef CONFIG_FIBOCOM_BASE  // SPCSS01514332
#ifdef CONFIG_KERNEL_MULTIPLE_HEAP_SUPPORT
    void *user_heap_start = (void *)(heap_start + heap_size);
    uint32_t user_heap_size = USER_HEAP_SIZE;
    gOsiUserHeap = osiBlockPoolInit(user_heap_start, user_heap_size, 0);
    OSI_LOGI(0, "user ram heap start/%p size/%d pool/%p", user_heap_start, user_heap_size, gOsiUserHeap);
#endif
#endif
}
...
OSI_NO_RETURN void osiWakePm1(void)
{
    ...
    hwp_gpio1->gpio_clr_reg = (1 << 15 | 1 << 14 );
...
#ifdef CONFIG_8850CM_TARGET
        uint32_t SlpTicks = UINT32_MAX;
        uint32_t SlpDurPs = UINT32_MAX;
        uint32_t MinDurTicks = UINT32_MAX;
