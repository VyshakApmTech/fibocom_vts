...
extern int32_t get_poweroff_mode(uint8_t *mode);
...
#define EXT_FLASH_EXAMPLE_ON_VIO_83M
        ...
        // 切记！！！一定要！！！根据客户示波器查引脚pin脚波形情况后，按需调整pin脚驱动电流等级，等级为0~15
        // 研发这里仅作demo指导演示
        halIomuxSetPadDriving(HAL_IOMUX_PAD_SPI_LCD_SIO, 8);
        halIomuxSetPadDriving(HAL_IOMUX_PAD_SPI_LCD_SDC, 8);
        halIomuxSetPadDriving(HAL_IOMUX_PAD_SPI_LCD_CLK, 8);
        halIomuxSetPadDriving(HAL_IOMUX_PAD_SPI_LCD_CS, 8);
        halIomuxSetPadDriving(HAL_IOMUX_PAD_SPI_LCD_SELECT, 8);
        halIomuxSetPadDriving(HAL_IOMUX_PAD_LCD_FMARK, 8);
...
//2024-08-01, kangmengjiang, MTC0745-440.End.
    ...
    halPmuSwitchPower(HAL_POWER_SD, true, false);
#elif defined CONFIG_FIBOCOM_PMU_SD_ENABLE
    halPmuSwitchPower(HAL_POWER_SD, true, true);
    halPmuSetPowerLevel(HAL_POWER_SD,3200);
#else
...
OSI_NO_RETURN void halShutdown(int mode, int64_t wake_uptime)
{
...
#elif defined(CONFIG_FIBOCOM_LOW_VOLTAGE_NORMAL_POWEROFF)
    if (mode == OSI_SHUTDOWN_LOW_VOLTAGE_POWER_OFF)
    {
...
#else
    uint8_t poweroff_mode = 0;
    if (get_poweroff_mode(&poweroff_mode) == 0)
    {
        OSI_LOGD(0, "poweroff_mode:%u", poweroff_mode);
    }
    else
    {
        OSI_LOGD(0, "get poweroff_mode failed");
    }
    if (mode == OSI_SHUTDOWN_POWER_OFF && poweroff_mode == 1)
    {
        //need to set dcxo lp,otherwise 400ua electric leakage when power off
        REG_RF_DIG_RTC_REG_C4_REG_T reg_c4_reg = {};
        reg_c4_reg.b.xtal_osc_ibit_n = 8;
    ...
    if (mode == OSI_SHUTDOWN_POWER_OFF)
    {
        ...
        halAdiBusWrite(&hwp_pmicPsm->psm_ctrl, psm_ctrl.v);
        halPmuSwitchPower(HAL_POWER_VIO18, false, false);
        // power off rtc
...
int _bootGetPowerOnCause(void)
{
	...
	OSI_LOGI(0, "osiGetBootCauses boot_causes:0x%x", boot_causes);
