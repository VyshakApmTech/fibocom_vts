...
static void prvGc9305SetDir(void *d, drvLcdDirection_t dir)
{
...
static void prvGc9305Init(void *d)
{
...
static void prvGc9305BlitPrepare(void *d, drvLcdDirection_t dir, const drvLcdArea_t *roi)
{
    OSI_LOGD(0x10007f57, "GC9305 dir/%d roi/%d/%d/%d/%d", dir, roi->x, roi->y, roi->w, roi->h);
...
static uint32_t prvGc9305ReadId(void *d)
{
    uint8_t id[4] = {0, 0, 0, 0};
    drvLcdReadData(d, LCD_CMD_READ_ID, id, 4);

    uint32_t dev_id = (id[3] << 16) | (id[2] << 8) | id[1];
    OSI_LOGI(0x10007f58, "GC9305 read id: 0x%08x", dev_id);
    return dev_id;
}

static bool prvGc9305Probe(void *d)
{
    const drvLcdPanelDesc_t *desc = drvLcdGetDesc(d);

    OSI_LOGI(0x10007f59, "GC9305 probe");
