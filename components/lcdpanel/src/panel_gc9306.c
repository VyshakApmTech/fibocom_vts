...
static void prvGc9306SetDir(void *d, drvLcdDirection_t dir)
{
...
static void prvGc9306Init(void *d)
{
...
static void prvGc9306BlitPrepare(void *d, drvLcdDirection_t dir, const drvLcdArea_t *roi)
{
    OSI_LOGD(0x1000b343, "GC9306 dir/%d roi/%d/%d/%d/%d", dir, roi->x, roi->y, roi->w, roi->h);

    prvGc9306SetDir(d, dir);
...
static uint32_t prvGc9306ReadId(void *d)
{
    uint8_t id[4] = {0, 0, 0, 0};
    drvLcdReadData(d, LCD_CMD_READ_ID, id, 4);

    uint32_t dev_id = (id[3] << 16) | (id[2] << 8) | id[1];
    OSI_LOGI(0x1000b344, "GC9306 read id: 0x%08x", dev_id);
    return dev_id;
}

static bool prvGc9306Probe(void *d)
{
    const drvLcdPanelDesc_t *desc = drvLcdGetDesc(d);

    OSI_LOGI(0x1000b345, "GC9306 probe");
    return prvGc9306ReadId(d) == desc->dev_id;
}

const drvLcdPanelDesc_t gLcdGc9306Desc = {
    .ops = {
        .probe = prvGc9306Probe,
        .init = prvGc9306Init,
        .blit_prepare = prvGc9306BlitPrepare,
    },
    .name = "GC9306",
    .dev_id = 0x009306,
    .reset_us = 20 * 1000,
