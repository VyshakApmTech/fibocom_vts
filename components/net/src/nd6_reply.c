...
//support DHCPv6
static u8_t dhcpv6_info_req_reply[] = {
...
static u8_t dhcpv6_info_dns[] = {
    ...
    0x20, 0x01, 0x48, 0x60, 0x48, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x88, 0x88, /* DNS address 1 */
    0x20, 0x01, 0x48, 0x60, 0x48, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x88, 0x44  /* DNS address 2 */
};
u8_t buffer_reply[200] = {0};
...
uint16_t Dhcp6_info_reply_dataprepare(uint8_t *pULDataBuf, uint8_t *pDLDataBuf, struct netif *netif)
{
    uint16_t dataLen = 0;

    if (pULDataBuf == NULL || pDLDataBuf == NULL || netif == NULL)
        goto LEAVE;

    uint8_t *dhcp6_reply = pDLDataBuf; //tx_req->payload->data; //p->payload;
    memcpy(dhcp6_reply, dhcpv6_info_req_reply, sizeof(dhcpv6_info_req_reply));

    //build src ip and dst ip
    //memcpy(dhcp6_reply + 8, pb->payload + 8 + 16, 16); //src ip
    memcpy(dhcp6_reply + 8 + 16, pULDataBuf + 8, 16); //dst ip

    //prepare XID CID
    uint8_t *dhcpv6 = dhcp6_reply + 48;
    uint8_t *dhcpv6_ul = pULDataBuf + 48; //Transaction ID

    uint16_t dhcp6_ul_len = 0;
    memcpy(&dhcp6_ul_len, pULDataBuf + 44, 2);
    OSI_LOGI(0x10009507, "nd6 dhcp6_ul_len = %d", lwip_ntohs(dhcp6_ul_len));
    dhcp6_ul_len = lwip_ntohs(dhcp6_ul_len);

    uint8_t *dhcpv6_ul_tag = pULDataBuf + 48 + 4; //frist Tag
    uint16_t subtaglen = dhcp6_ul_len - (8 + 4);
    OSI_LOGI(0x10009508, "nd6 subtaglen = %d", subtaglen);

    //find CID from UL data
    uint8_t *pTag = dhcpv6_ul_tag;
    uint16_t u16Option = 0;
    uint16_t u16OptionLen = 0;
    while (subtaglen > 0)
    {
        //Get Options
        memcpy(&u16Option, pTag, 2);
        u16Option = lwip_ntohs(u16Option);
        OSI_LOGI(0x10009509, "nd6 pTag = %d", u16Option);

        //Get Options len
        memcpy(&u16OptionLen, pTag + 2, 2);
        u16OptionLen = lwip_ntohs(u16OptionLen);
        OSI_LOGI(0x1000950a, "nd6 u16OptionLen = %d", u16OptionLen);

        if (u16Option == 0x0001) //find Client ID Options from UL
        {
            dhcpv6_ul_tag = pTag;
            break;
        }
        ...
        //point to next Tag
        subtaglen -= u16OptionLen + 2 + 2; // 2 byte Option + 2 byte length
        pTag += u16OptionLen + 2 + 2;
    }

    if (u16Option != 0x0001)
        goto LEAVE; //no Clinet ID

    sys_arch_dump(dhcpv6_ul_tag, u16OptionLen + 2 + 2);
    uint16_t cid_len = u16OptionLen + 2 + 2;
    OSI_LOGI(0x1000950b, "nd6 cid_len = %d", cid_len);
    if (cid_len == 0)
        goto LEAVE;

    memcpy(dhcpv6 + 1, dhcpv6_ul + 1, 3);       //copy Transaction ID
    memcpy(dhcpv6 + 4, dhcpv6_ul_tag, cid_len); //copy CID

    //DNS recursive name Server
    CFW_GPRS_PDPCONT_INFO_V2 sPdpCont = {0};
    OSI_LOGI(0x10007ad6, "dhcpv6 sim = %d, cid = %d", (netif->sim_cid >> 4), (netif->sim_cid & 0x0F));
    ...
    OSI_LOGI(0x10007543, "DNS size:%d  ipv6 DNS 12-15:%d.%d.%d.%d", sPdpCont.nPdpDnsSize, sPdpCont.pPdpDns[12], sPdpCont.pPdpDns[13], sPdpCont.pPdpDns[14], sPdpCont.pPdpDns[15]);
    OSI_LOGI(0x10007544, " ipv6 DNS 33-36:%d.%d.%d.%d", sPdpCont.pPdpDns[33], sPdpCont.pPdpDns[34], sPdpCont.pPdpDns[35], sPdpCont.pPdpDns[36]);

    IP6_ADDR(&ip6addr_dns, addr0, addr1, addr2, addr3);
    if ((addr0 | addr1 | addr2 | addr3) != 0)
    memcpy(dhcpv6_info_dns + 18, &(ip6addr_dns), sizeof(ip6_addr_p_t));

    IP6_ADDR(&ip6addr_dns, addr4, addr5, addr6, addr7);
    if ((addr4 | addr5 | addr6 | addr7) != 0)
    memcpy(dhcpv6_info_dns + 18 + 16, &(ip6addr_dns), sizeof(ip6_addr_p_t));

    //copy info dns
    memcpy(dhcpv6 + 3 + cid_len + 1, dhcpv6_info_dns, sizeof(dhcpv6_info_dns));

    dataLen = sizeof(dhcpv6_info_req_reply) + cid_len + sizeof(dhcpv6_info_dns);
    OSI_LOGI(0x1000950c, "nd6 dataLen = %d %d %d", sizeof(dhcpv6_info_req_reply), cid_len, sizeof(dhcpv6_info_dns));

    //update udp len
    uint16_t u16udplen = lwip_htons(dataLen - 40);
    OSI_LOGI(0x1000950d, "nd6 udp reply len 0x%x", u16udplen);
    memcpy(dhcp6_reply + 44, &u16udplen, 2);

    //update ipv6 header payload len
    OSI_LOGI(0x1000950e, "nd6 ipv6 payload len 0x%x", u16udplen);
    memcpy(dhcp6_reply + 4, &u16udplen, 2);
LEAVE:
    return dataLen;
}
void ND6_Dhcp6_Info_req_reply(drvEther_t *ether, drvEthPacket_t *pkt, uint32_t size, struct netif *netif)
{
    ...
    uint8_t *dhcp6_reply = tx_req->payload->data;

    dataLen = Dhcp6_info_reply_dataprepare(pkt->data, dhcp6_reply, netif);
    if (dataLen == 0)
    {
        drvEtherTxReqFree(ether, tx_req);
        return;
    ...
    sys_arch_dump(dhcp6_reply, dataLen);
    drvEtherTxReqSubmit(ether, tx_req, dataLen);
}
