...
extern void ip4_nat_devWanAct(void);
extern void ip4_nat_devWanDeAct(struct netif *pWan);
#endif
    ...
    for (;;)
    {
        ...
        else
        {
...
#if defined(CONFIG_SOC_8910) || defined(CONFIG_SOC_8850)
#ifdef CONFIG_PAM_LTE_GNSS_WIFISCAN_SUPPORT
                    if (netdevIsConnected() && (nCid != (pamGetAgnssActPdp() >> 4)))
#else
                    if (netdevIsConnected())
#endif
                    {
            ...
            case EV_TCPIP_CFW_GPRS_DEACT:
            {
                ...
                {
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT
                    ip4_nat_devWanDeAct(getWan(nSimId, nCid)->netif);
#endif
                    destroyWan(nSimId, nCid);
                }
                else
...
void net_init()
{
    ...
    netThreadID = osiThreadCreateForCore("net", net_thread, NULL, OSI_HIGHER_PRIORITY_NORMAL, NET_STACK_SIZE, 64);
#else
