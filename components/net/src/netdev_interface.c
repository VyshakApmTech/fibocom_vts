...
static void prvNdevProcessPsData(void *ctx)
{
    ...
    if (nc->netdevIsSuspend == true)
    {
        OSI_LOGI(0, "prvNdevProcessPsData nc->netdevIsSuspend1 == true");    //MTC0803-416
        return;
    }
    ...
    do
    {
        ...
        while ((nc->netdevIsSuspend == false) && ((tx_req = drvEtherTxReqAlloc(nc->ether)) == NULL))
        {
            ...
            if (nc->ether == NULL)
            {
                ...
                /*MTC0803-416 start*/
                OSI_LOGI(0, "prvNdevProcessPsData nc->ether == NULL");
                uint8_t *temp_rdata = (uint8_t *)malloc(1600);
                rsize = drvPsIntfRead(nif->pspathIntf, temp_rdata, 1600);
                while(rsize > 0)
                {
                    rsize = drvPsIntfRead(nif->pspathIntf, temp_rdata, 1600);
                    OSI_LOGI(0, "prvNdevProcessPsData drvPsIntfRead drain chain rsize %d", rsize);
                }
                free(temp_rdata);
                /*MTC0803-416 end*/
                return;
        ...
        if (nc->netdevIsSuspend == true)
        {
            ...
            if (tx_req != NULL)
                drvEtherTxReqFree(nc->ether, tx_req);
            OSI_LOGI(0, "prvNdevProcessPsData nc->netdevIsSuspend2 == true");    //MTC0803-416
            return;
        }
        if (nc->mutex != NULL)
        {
        ...
        OSI_LOGI(0, "prvNdevProcessPsData drvPsIntfRead enter");    //MTC0803-416
        if (rsize <= 0)
        {
            drvEtherTxReqFree(nc->ether, tx_req);
        }
