...
#endif
#endif
#ifdef CONFIG_FIBOCOM_BASE  // SPCSS01514332
#include "osi_mem.h"
#endif
...
static void gprs_data_ipc_to_lwip_nat_wan(void *ctx)
{
    ...
    do
    {
        ...
        {
            p = (struct pbuf *)pbuf_alloc(PBUF_LINK, len, PBUF_POOL);
            offset = 0;
            if (p != NULL)
            {
                if (len > p->len)
...
#ifndef CONFIG_FIBOCOM_BASE  // SPCSS01514332
...
#ifdef CONFIG_FIBOCOM_BASE  // SPCSS01514332
    struct ps_header *psHeader = (struct ps_header *)osiPoolMemalign(gOsiDefaultHeap, 32, ps_len_align);
#else
...
u8_t ip4_wan_forward(struct pbuf *p, struct netif *inp)
{
    ...
    if (IP_HDR_GET_VERSION(p->payload) == 4)
    {
        ...
        NETIF_FOREACH(netif)
        {
            ...
            if (NETIF_LINK_MODE_NAT_WIFI == netif->link_mode)
            {
                static ip4_addr_t wifi_gw_mask = {
                    .addr = PP_HTONL(LWIP_MAKEU32(255, 255, 255, 0))};

                struct ip_hdr *iphdr = p->payload;
                ip4_addr_t current_iphdr_src;
                ip4_addr_t current_iphdr_dest;
                ip4_addr_t *current_netif_addr;
                ip4_addr_copy(current_iphdr_src, iphdr->src);
                ip4_addr_copy(current_iphdr_dest, iphdr->dest);
                current_netif_addr = (ip4_addr_t *)netif_ip4_addr(netif);
                if ((current_iphdr_dest.addr & wifi_gw_mask.addr) == (current_netif_addr->addr & wifi_gw_mask.addr) || (current_iphdr_dest.addr == PP_HTONL(LWIP_MAKEU32(255, 255, 255, 255)) && (current_iphdr_src.addr != current_netif_addr->addr)))
                {
                    OSI_LOGI(0, "ip4_wan_forward IPV4 to wifi gateway");
                    pbuf_ref(p);
                    netif->input(p, netif);
                    taken = 1;
                    break;
...
uint8_t lantype[NAT_MAX_LANNUM] = {NETIF_LINK_MODE_NAT_LWIP_LAN, NETIF_LINK_MODE_NAT_NETDEV_LAN, NETIF_LINK_MODE_NAT_PPP_LAN, NETIF_LINK_MODE_NAT_WIFI};
uint8_t lanip[NAT_MAX_LANNUM] = {NAT_IPADDR_LWIP, NAT_IPADDR_RNDIS, NAT_IPADDR_PPP, NAT_IPADDR_WIFI};
...
struct netif *nat_lan_netif_create(uint8_t nCid, uint8_t nSimId, uint8_t nIdx, netif_init_fn init, netif_input_fn input)
{
    ...
    if ((nIdx == NAT_LAN_IDX_RNDIS) || (nIdx == NAT_LAN_IDX_LWIP && nSimId == NAT_SIM && nCid == NAT_CID))
    {
        ...
        IP4_ADDR(&ip4, 192, 168, 225, 100);
        IP4_ADDR(&ip4_gw, 192, 168, 225, 1);
#elif defined CONFIG_FIBO_CUS_HZHC_FEATURE
...
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT
    else if (nIdx == NAT_LAN_IDX_WIFI)
    {
        IP4_ADDR(&ip4, 192, 168, 100, 1);
        IP4_ADDR(&ip4_gw, 192, 168, 100, 1);
    }
#endif
    else
    {
...
#endif
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT
    ip4_nat_entry_netif_t *cur = netif->nat_entry;
    if (NULL != cur)
    {
        ip4_nat_entry_netif_t *next = NULL;

        ip4_nat_remove_netif(netif);
...
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT
extern void dhcps_dns_setserver(int cid, int sim);
#endif
...
void wan_unbindLan(void *wannetif, uint8_t nSimId, uint8_t nCid)
{
    ...
    while (pwan->lan_netiflist != NULL)
    {
        ...
        if (plan != NULL)
        {
            ...
            if (iUnique == true)
            {
                if ((nType == NAT_LAN_IDX_RNDIS) || (nType == NAT_LAN_IDX_WIFI))
                {
...
void wan_bindLan(void *wannetif)
{
    ...
    while (plan != NULL)
    {
        ...
        if (((plan_netif->pdnType == CFW_GPRS_PDP_TYPE_PPP + 1) && (plan_netif->link_mode != NETIF_LINK_MODE_NAT_LWIP_LAN)) || (plan_netif->link_mode == NETIF_LINK_MODE_NAT_WIFI))
        {
...
void destroyWan(uint8_t nSimId, uint8_t nCid)
{
    ...
    if (iUniqe == true)
    {
        ...
        if (plan_lwip == NULL || plan == NULL)  //MTC0587-1015
            goto end;
