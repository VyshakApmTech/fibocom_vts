...
#ifdef CONFIG_FIBOCOM_CC_LWM2M_SUPPORT
void conn_s_updateTxStatistic(uint16_t txDataByte, bool smsBased);
void conn_s_updateRxStatistic(uint16_t rxDataByte, bool smsBased);
#endif
...
#ifdef CONFIG_FIBOCOM_CC_LWM2M_SUPPORT
    conn_s_updateRxStatistic(p->tot_len, false);
#endif
...
#ifdef CONFIG_FIBOCOM_CC_LWM2M_SUPPORT
    conn_s_updateTxStatistic(p->tot_len, false);
#endif
...
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT
    if (NULL == netif->nat_entry)
    {
        netif->nat_entry = ip4_nat_cfg_new;
    }
    else
    {
