...
//2025-2-4 , weijy, MTC0587-1058, add callback for DCD status, the modify from 1.5 (MTC0700-524)
static fibo_ppp_set_dcd_cb g_fibo_ppp_set_dcd_cb = NULL;
fibo_ppp_set_dcd_cb fibo_get_dcd_cb()
{
    return g_fibo_ppp_set_dcd_cb;
}
#endif
#if LWIP_IPV6
...
#ifdef CONFIG_FIBOCOM_BASE
    if (!netif_is_valid(nif))    //MTC0587-899
    {
        OSI_LOGI(0, "_ppp_lan_data_pull nif invalid");
        return;
    }
#endif
    if (nif->state == NULL)
...
#ifdef CONFIG_FIBOCOM_BASE
    //SET DCD if active
    if(NULL != g_fibo_ppp_set_dcd_cb)
    {
        g_fibo_ppp_set_dcd_cb(FIBO_PPP_PDP_ACTIVE);
    }
#endif
...
static void _pppDeactiveRsp(pppSession_t *ppp, const osiEvent_t *event)
{
    ...
    if (cfw_event->nType == CFW_GPRS_DEACTIVED)
    {
        ...
        if(NULL != g_fibo_ppp_set_dcd_cb)
        {
            g_fibo_ppp_set_dcd_cb(FIBO_PPP_PDP_DEACTIVE);
        }
#endif
    }
}
...
bool pppSessionDelete(pppSession_t *ppp)
{
    ...
    {
        if (ppp->cgact_activated == 1)      //MTC0587-1084
        {
            uint16_t uti = cfwRequestUTI((osiEventCallback_t)_pppDeactiveRsp, ppp);
            OSI_LOGE(0, "ppp send CFW_GPRS_DEACTIVED");
...
void fibo_ppp_set_dcd(fibo_ppp_set_dcd_cb cb)
{
    g_fibo_ppp_set_dcd_cb = cb;
}
