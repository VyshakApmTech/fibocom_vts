...
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT
typedef struct ip4_nat_dev_info_t
{
    struct ip4_nat_dev_info_t *next;
    bool effect;
    bool dev_conn;
    bool ip_offer;
    struct netif *netif;
...
static ip4_nat_dev_info *g_ip4_nat_dev = NULL; /* todo sgy array or list */
static osiMutex_t *g_ip4_dev_Lock = NULL;

struct netif *ip4_nat_getWanifByLan(const struct netif *netif)
{
    ip4_nat_conf_t *cur = ip4_nat_cfg;
    struct netif *pWan = NULL;

    while (cur != NULL)
    {
        if (netif == cur->entry.in_if)
        {
            pWan = cur->entry.out_if;
            break;
        }
        cur = cur->next;
    }
...
static void ip4_nat_devIpValid(struct netif *netif, ip4_addr_t ip, uint8_t *mac)
{
    ip4_nat_dev_info *cur = g_ip4_nat_dev;
    ip4_nat_dev_info *new = NULL;
    struct netif *pLan = netif;
    struct netif *pWan = NULL;

    OSI_LOGI(0, "ip4_nat_devIpValid add netif:%p, ip:%#x\n", netif, ip.addr);

    osiMutexLock(g_ip4_dev_Lock);
    while (cur != NULL)
    {
    ...
    if (cur != NULL)
    {
        if ((true != cur->effect) &&
            (true == cur->dev_conn))
        {
            pWan = ip4_nat_getWanifByLan(pLan);
            cur->wan = pWan;
            if (ERR_OK == ip4_nat_addLanDev(pLan, pWan, ip))
            {
                OSI_LOGI(0, "ip4_nat_devIpValid add dev success\n");
                cur->effect = true;
            }
        }
    }
    ...
    {
        new = (ip4_nat_dev_info *)calloc(1, sizeof(ip4_nat_dev_info));
        if (NULL == new)
        {
            OSI_LOGE(0, "ip4_nat_devIpValid malloc failed\n");
            osiMutexUnlock(g_ip4_dev_Lock);
            return;
        }
...
static void ip4_nat_devIpExpired(struct netif *netif, ip4_addr_t ip, uint8_t *mac)
{
    ip4_nat_dev_info *cur = g_ip4_nat_dev;
    ip4_nat_dev_info *next;
    ip4_nat_dev_info *previous = NULL;

    OSI_LOGI(0, "ip4_nat_devIpExpired del netif:%p, ip:%#x\n", netif, ip.addr);
    ...
    while (cur != NULL)
    {
        if ((netif == cur->netif) &&
            (ip.addr == cur->ip.addr) &&
            (0 == memcmp(cur->mac, mac, sizeof(cur->mac))))
        {
            if (true == cur->effect)
            {
                ip4_nat_removeLanDev(netif, ip);
                cur->effect = false;
            }

            if (true == cur->ip_offer)
            {
                if (true == cur->dev_conn)
                {
                    cur->ip_offer = false;
                    cur->ip.addr = 0;
                }
...
static void ip4_nat_devAssoc(uint8_t *mac)
{
    ip4_nat_dev_info *cur = g_ip4_nat_dev;
    ip4_nat_dev_info *new = NULL;
    struct netif *pWan = NULL;
    ...
    while (cur != NULL)
    {
        if (0 == memcmp(cur->mac, mac, sizeof(cur->mac)))
        {
            OSI_LOGI(0, "ip4_nat_devAssoc find dev effect:%d, dev_conn:%d, ip_offer:%d, "
                     "mac:%x-%x-%x-%x-%x-%x\n", cur->effect, cur->dev_conn, cur->ip_offer,
    ...
    if (cur != NULL)
    {
        if ((true != cur->effect) &&
            (true == cur->ip_offer))
        {
            pWan = ip4_nat_getWanifByLan(cur->netif);
            cur->wan = pWan;
            if (ERR_OK == ip4_nat_addLanDev(cur->netif, pWan, cur->ip))
            {
                OSI_LOGI(0, "ip4_nat_devAssoc add dev success\n");
                cur->effect = true;
            }
        }
    }
    else
    {
        ...
        if (NULL == new)
        {
            OSI_LOGE(0, "ip4_nat_devAssoc malloc failed\n");
            osiMutexUnlock(g_ip4_dev_Lock);
            return;
        }
...
static void ip4_nat_devDisassoc(uint8_t *mac)
{
    ip4_nat_dev_info *cur = g_ip4_nat_dev;
    ip4_nat_dev_info *next;
    ip4_nat_dev_info *previous = NULL;
    ...
    while (cur != NULL)
    {
        if (0 == memcmp(cur->mac, mac, sizeof(cur->mac)))
        {
            if (true == cur->effect)
            {
                ip4_nat_removeLanDev(cur->netif, cur->ip);
                cur->effect = false;
            }
...
void ip4_nat_devWanAct(void)
{
    struct netif *pLan = NULL;
    struct netif *pWan = NULL;
    ip4_nat_dev_info *cur = NULL;

    OSI_LOGI(0, "ip4_nat_devWanAct process\n");

    osiMutexLock(g_ip4_dev_Lock);
    cur = g_ip4_nat_dev;
    while (cur != NULL)
    {
        if ((false == cur->effect) && (true == cur->ip_offer) && (true == cur->dev_conn))
        {
            pLan = cur->netif;
            pWan = ip4_nat_getWanifByLan(pLan);
            cur->wan = pWan;
            if (ERR_OK == ip4_nat_addLanDev(pLan, pWan, cur->ip))
            {
                cur->effect = true;
                OSI_LOGI(0, "ip4_nat_devWanAct add lan:%p, wan:%p, ip:%#x\n",
                         pLan, pWan, cur->ip.addr);
            }
            else
            {
                OSI_LOGI(0, "ip4_nat_devWanAct add failed lan:%p, wan:%p, ip:%#x\n",
                         pLan, pWan, cur->ip.addr);
            }
        }
        cur = cur->next;
...
void ip4_nat_devWanDeAct(struct netif *pWan)
{
    ip4_nat_dev_info *cur = NULL;
    ...
    while (cur != NULL)
    {
        if ((true == cur->effect) && (pWan == cur->wan))
        {
            ip4_nat_removeLanDev(cur->netif, cur->ip);
            cur->effect = false;
            cur->wan = NULL;
            OSI_LOGI(0, "ip4_nat_devWanDeAct remove wan:%p, ip:%#x\n",
                     pWan, cur->ip.addr);
        }
        cur = cur->next;
    }
...
void ip4_nat_init(void)
{
    ...
    g_ip4_dev_Lock = osiMutexCreate();
    osiExitCritical(critical);
    if (NULL == g_ip4_dev_Lock)
    {
        LWIP_DEBUGF(NAT_DEBUG, (0x100097fa, "ip4_nat_init: no more memery for g_ip4_dev_Lock\n"));
        free(ip4_nat_entries_udp_header);
        free(ip4_nat_entries_tcp_header);
        free(ip4_nat_entries_icmp_header);
        return;
...
ip4_nat_conf_t *ip4_nat_add(const ip4_nat_entry_t *new_entry)
{
    ...
    if (ip4_nat_cfg_new != NULL)
    {
        ...
        {
#ifdef CONFIG_FIBOCOM_BASE
            LWIP_DEBUGF(NAT_DEBUG, (0, "ip4_nat_add: first add\n"));
#endif
...
#ifdef CONFIG_FIBOCOM_BASE
            //MTC0803-496 This change is to fix a DUMP where the cur fails to update to NULL even after ip4_nat_cfg is assigned elsewhere 
            ip4_nat_conf_t *cur = ip4_nat_cfg;
            LWIP_DEBUGF(NAT_DEBUG, (0, "ip4_nat_add: foreset for port_index,cur %p\n",cur));
#endif
...
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT
err_t ip4_nat_addLanDev(struct netif *lan_netif, struct netif *wan_netif, ip4_addr_t ip4_addr)
{
    err_t ret = ERR_OK;
    OSI_LOGI(0, "ip4_nat_addLanDev lannetif = %p, wannetif = %p, ip:%#x",
             lan_netif, wan_netif, ip4_addr.addr);
    ...
    ip4_nat_entry_netif_t *ip4_nat_cfg_new =
                           (ip4_nat_entry_netif_t *)calloc(1, sizeof(ip4_nat_entry_t));
    if (NULL == ip4_nat_cfg_new)
    {
        ret = ERR_MEM;
        goto end;
    }

    ip4_nat_entry_t *new_nat_entry = &ip4_nat_cfg_new->entry;
    ...
    ip4_addr_copy(new_nat_entry->source_net, ip4_addr);
    ...
    if (NULL == lan_netif->nat_entry)
    {
        lan_netif->nat_entry = ip4_nat_cfg_new;
    }
    else
    {
        ip4_nat_entry_netif_t *cur = lan_netif->nat_entry;
        while (cur->next != NULL)
        {
            cur = cur->next;
        }
    ...
    OSI_LOGI(0, "ip4_nat_addLanDev iRet = %d", ret);
    return ret;
}
#endif
...
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT
/** Delete a NAT entry for a dev connect to netif
 *
 * @return ERR_OK if succeeded
 */
err_t ip4_nat_removeLanDev(struct netif *lannetif, ip4_addr_t ip4_addr)
{
    err_t ret = ERR_OK;
    ...
    ip4_nat_entry_netif_t *cur = lannetif->nat_entry;
    ip4_nat_entry_netif_t *next = NULL;
    ip4_nat_entry_netif_t *previous = NULL;
    while (cur != NULL)
    {
        if ((cur->entry.source_net.addr == ip4_addr.addr) &&
            (cur->entry.in_if == lannetif))
        {
            ip4_nat_remove(&cur->entry);
            next = cur->next;
            if (cur == lannetif->nat_entry)
            {
                lannetif->nat_entry = next;
            }
            else
            {
                LWIP_ASSERT("NULL != previous", NULL != previous);
    ...
    OSI_LOGI(0, "ip4_nat_removeLanDev iRet = %d", ret);
    return ret;
}

/** Remove all NAT entry for a netif previously added by 'ip4_nat_add()'.
 *
 * @param remove_entry describes the entry to remove
 */
void ip4_nat_remove_netif(void *netif)
{
    ip4_nat_conf_t *cur = ip4_nat_cfg;
    ip4_nat_conf_t *next = NULL;
    ip4_nat_conf_t *previous = NULL;
    struct netif *lan_if = (struct netif *)netif;

    if (NULL == netif)
    {
        LWIP_DEBUGF(NAT_DEBUG, (0, "ip4_nat_remove_netif netif ==  NULL\n"));
        return;
    }
...
#define DHCPS_IP_ADDR_EXPIRED (0)
#define DHCPS_IP_ADDR_VALID (1)

typedef bool (*dhcps_ip_status_cb_t)(struct netif *netif, u8_t flag, ip4_addr_t ip, u8_t *mac);

bool dhcp_ip_status_cb(struct netif *netif, u8_t flag, ip4_addr_t ip, u8_t *mac)
{
    struct netif *pLan = netif;
    bool ret = false;
    ...
    OSI_LOGI(0, "dhcp_ip_status_cb recv dhcp event :%d, netif:%p, ip:%#x, mac:%x-%x-%x-%x-%x-%x\n",
             flag, netif, ip.addr, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

    if (DHCPS_IP_ADDR_EXPIRED == flag)
    {
        ip4_nat_devIpExpired(pLan, ip, mac);
    }
    else if (DHCPS_IP_ADDR_VALID == flag)
    {
        ip4_nat_devIpValid(pLan, ip, mac);
    }
    else
    {
        /* todo sgy log */
...
void ip4_nat_wifi_event_callback(SSV_WIFI_EVENT enEvent, ssv_wifi_event_data *enData)
{
    uint8_t mac[6] = {0};
    ...
    switch(enEvent)
    {
        ...
        case ASSOC_IND_EVENT:
        {
            ...
            ip4_nat_devAssoc(mac);
            break;
        }
        case DISASSOC_IND_EVENT:
        {
            mac[0] = enData->data.auth_deauth_data.reserved[0];
            mac[1] = enData->data.auth_deauth_data.reserved[1];
            mac[2] = enData->data.auth_deauth_data.reserved[2];
    ...
    OSI_LOGI(0, "ip4_nat_wifi_event_callback recv wifi event :%d, mac:%x-%x-%x-%x-%x-%x\n",
             enEvent, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
...
void ip4_nat_displayNatCfg(atCmdEngine_t *engine)
{
    ip4_nat_conf_t *cur = ip4_nat_cfg;
    char buf[128] = {0};
    ip4_addr_t *addr = NULL;

    atCmdRespDefUrcText("ip4 conf info Urc");
    atCmdRespInfoText(engine, "ip4 conf info eng");
    while (cur != NULL)
    {
        atCmdRespDefUrcText("ip4 conf info");
        addr = &(cur->entry.source_net);
        snprintf(buf, 128, "src ip:%hu.%hu.%hu.%hu",
            ip4_addr1(addr), ip4_addr2(addr), ip4_addr3(addr), ip4_addr4(addr));
//        atCmdRespDefUrcText(buf);
        atCmdRespInfoText(engine, buf);
        addr = &(cur->entry.source_netmask);
        snprintf(buf, 128, "src mask:%hu.%hu.%hu.%hu",
            ip4_addr1(addr), ip4_addr2(addr), ip4_addr3(addr), ip4_addr4(addr));
        //atCmdRespDefUrcText(buf);
        atCmdRespInfoText(engine, buf);
        addr = ip_2_ip4(&(cur->entry.in_if->ip_addr));
        snprintf(buf, 128, "in if ip:%hu.%hu.%hu.%hu",
            ip4_addr1(addr), ip4_addr2(addr), ip4_addr3(addr), ip4_addr4(addr));
        //atCmdRespDefUrcText(buf);
        atCmdRespInfoText(engine, buf);
        addr = &(cur->entry.dest_net);
        snprintf(buf, 128, "dest ip:%hu.%hu.%hu.%hu",
            ip4_addr1(addr), ip4_addr2(addr), ip4_addr3(addr), ip4_addr4(addr));
        //atCmdRespDefUrcText(buf);
        atCmdRespInfoText(engine, buf);
        addr = &(cur->entry.dest_netmask);
        snprintf(buf, 128, "dest mask:%hu.%hu.%hu.%hu",
            ip4_addr1(addr), ip4_addr2(addr), ip4_addr3(addr), ip4_addr4(addr));
        //atCmdRespDefUrcText(buf);
        atCmdRespInfoText(engine, buf);
        addr = ip_2_ip4(&(cur->entry.out_if->ip_addr));
        snprintf(buf, 128, "out_if ip:%hu.%hu.%hu.%hu",
            ip4_addr1(addr), ip4_addr2(addr), ip4_addr3(addr), ip4_addr4(addr));
       // atCmdRespDefUrcText(buf);
        atCmdRespInfoText(engine, buf);
        atCmdRespInfoText(engine, "**************************");
...
static ip4_nat_conf_t *ip4_nat_shallnat_for_inputpkt(const struct ip_hdr *iphdr)
{
    ...
    OSI_PRINTFE("[fibocom] ECM_HOSTLANIP nv ip is %d, IP addr is %s", val[0], ip_addr_t1);
    ip4addr_aton(ip_addr_t1, &temp_ip);
#else
...
#ifdef CONFIG_NET_NAT_WIFI_SUPPORT    
    ip4_addr_t src_addr = {0};
    ip4_addr_t gw_addr = {0};
    iphdr = (struct ip_hdr *)p->payload;
    ip4_addr_copy(src_addr, iphdr->src);
#if 0
    struct ip_hdr tmp_iphdr = {0};	
    static ip4_addr_t wifi_gw_addr = {
                    .addr = PP_HTONL(LWIP_MAKEU32(192, 168, 100, 0))};
#endif
    ip4_addr_set_u32(&gw_addr, PP_HTONL(0xc0a86401UL));
#endif
