...
function (mkcpsimage memlist_file src dst)
    execute_process(COMMAND python3 ${CMAKE_CURRENT_SOURCE_DIR}/tools/getcpentry.py ${memlist_file}
...
function(pacvariant_gen)
    ...
    set(cp_dir_name ${BUILD_TARGET})
    foreach(nvmvariant ${CONFIG_NVM_VARIANTS})
        set(cp_dir_name ${nvmvariant})
        break()  #only get the first name
    endforeach()
    #Insert simlock key to CP bin
    set(srcmodembin ${MODEM_ROOT_DIR}/${cp_dir_name}/cp.bin)
    set(cpsimlockbin ${out_hex_dir}/cp-simlock.bin)
    if(CONFIG_FUNC_SUPPORT_SIMLOCK_CP)
        message("-- Insert ${cp_dir_name} SimlocKey")
        set(CURR_CMD_CP_SIMLOCK ${cmd_insertsimlockey} ${cpsimlockeydir} ${srcmodembin} ${cpsimlockbin})
    else()
        set(CURR_CMD_CP_SIMLOCK ${CMAKE_COMMAND} -E copy ${srcmodembin} ${cpsimlockbin})
    endif()
    add_custom_command(
        COMMAND ${CURR_CMD_CP_SIMLOCK}
        OUTPUT ${cpsimlockbin}
    )
    ...
    if(CONFIG_CP_SIGN_ENABLE)
        mkcpsimage(${MODEM_ROOT_DIR}/${cp_dir_name}/memory_index_list.bin ${cpsimlockbin} ${cpsimage})
        sign_image(${cpsimage}  ${cpsign} ${with_chaincert} false false)
    else()
        add_custom_command(
            COMMAND ${CMAKE_COMMAND} -E copy ${cpsimlockbin} ${cpsign}
            OUTPUT ${cpsign}
            DEPENDS ${cpsimlockbin}
        )
    endif()
    if(CONFIG_TRUSTZONE_SUPPORT)
        if(CONFIG_TRUSTZONE_USE_SPEC_SML_TOS)
            #use specific img instead of temp generated img 
            if(NOT TRUSTZONE_SPEC_ROOT_DIR)
                set(TRUSTZONE_SPEC_ROOT_DIR ${SOURCE_TOP_DIR}/${CONFIG_TRUSTZONE_SPEC_SML_TOS_DIR})
                message(STATUS "sml dir ${TRUSTZONE_SPEC_ROOT_DIR}")
            endif()
            configure_file(${TRUSTZONE_SPEC_ROOT_DIR}/sml.img  ${out_hex_dir}/sml.img COPYONLY)
            configure_file(${TRUSTZONE_SPEC_ROOT_DIR}/sml.sign.img  ${out_hex_dir}/sml.sign.img COPYONLY)
            configure_file(${TRUSTZONE_SPEC_ROOT_DIR}/tos.img  ${out_hex_dir}/tos.img COPYONLY)
            configure_file(${TRUSTZONE_SPEC_ROOT_DIR}/tos.sign.img  ${out_hex_dir}/tos.sign.img COPYONLY)
        else()
