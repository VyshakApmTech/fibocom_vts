...
#define UICC_EF_STATUS_INFO_SIZE 25
...
#define USIM_APP_LABEL_MAX_LEN 32
struct USIM_AID_INFO
{
    ...
    uint8_t label[USIM_APP_LABEL_MAX_LEN];
    uint8_t aid[16];
};
...
#define USIM_ADN_EF_SUPPORT_NUM 2

#define UICC_ECC_MAX_NUM 20
#define UICC_PL_MAX_NUM 5
#define UICC_LI_MAX_NUM 5

struct UICC_LANG
{
    uint8_t l2f05[UICC_PL_MAX_NUM][2]; //pl for usim, elp for sim
    uint8_t l6f05[UICC_LI_MAX_NUM][2]; //li for usim, lp for sim
};
...
#define UICC_GID_MAX_SIZE 128
struct GID_INFO
{
    uint8_t size;
    uint8_t value[UICC_GID_MAX_SIZE];
};
...
#define UICC_BOOT_STAGE_WAIT_VERIFY_PIN 0x80
...
struct UICC_CORE_INFO
{
    ...
    uint8_t uicc_mode;              //soft sim/cloud sim and so on
...
uint32_t CFW_SetActivedUiccNum(void);
...
bool uicc_get_ef_info(struct UICC_EF_INFO *info, uint8_t ef_index, CFW_SIM_ID nSimID);
