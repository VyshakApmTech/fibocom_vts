...
#define CFW_EVENT_STRING_SZ 128
...
+--------------------+-----------------------------------------------------+
\endrst
\param [in] pUserData Pointer to user data. 用户的数据指�?该参数会传入注册的EventProc 函数去处�?
...
#define CFW_API_STOP_TYPE_NORMAL 0x00
#define CFW_API_STOP_TYPE_CLOSE_SIM 0x01
#define CFW_API_STOP_TYPE_AIRMODE 0x02
#define CFW_API_STOP_TYPE_RAT_CHANGE 0x04
#define CFW_API_STOP_TYPE_LOCAL_DETACH 0x08
...
#define CFW_PDPCID_MIN 1
#ifdef CFW_VOLTE_SUPPORT
#define CFW_PDPCID_MAX 11
#else
...
typedef struct _CFW_IMS_ASSOCIATED_INFO
{
    uint8_t nLength;
    char p_associated_uri[CFW_EVENT_STRING_SZ + 1];
} CFW_IMS_ASSOCIATED_INFO;
...
#define CFW_BLACK_WHITE_PLMN_NUM 30
typedef struct
{
...
uint32_t CFW_GetIgnoreSib1Edrx(CFW_SIM_ID nSimID);
uint32_t CFW_SetIgnoreSib1Edrx(uint32_t value, CFW_SIM_ID nSimID);
...
| Value |                                     Description                                      |
+=======+======================================================================================+
|       | MS 返回 Error(nUsdStringSize标识 Errsource , pUsdString[0]标识具体�?ErrCode);       |
| 0     | USSD version 2 operation                                                             |
+-------+--------------------------------------------------------------------------------------+
|       | MS返回拒绝 (nUsdStringSize标识 ProblemCodeTag ,pUsdString[0]标识具体�?ProblemCode)  |
...
+-------+--------------------------------------------------------------------------------------+
|       |                                                                                      |
|       | MS 返回 Error(nUsdStringSize标识 Errsource, pUsdString[0]标识具体�?ErrCode );USSD   |
...
uint32_t CFW_SimWritePreferPLMN(uint8_t index, uint8_t *operater, uint8_t nSize, uint16_t nUTI, CFW_SIM_ID nSimID);
...
uint32_t CFW_EmodSetFreqB8OptEnable(uint8_t bEnable, CFW_SIM_ID nSimID);
uint32_t CFW_EmodGetFreqB8OptEnable(uint8_t *pEnable, CFW_SIM_ID nSimID);
...
#define AT_CFGFN_AUTOCONNECT "./AT_autoconnect.nv"
#if defined CONFIG_FIBOCUS_HAIKANG_FEATURE
#define AT_CFGFN_PORTMODE "./AT_portmode.nv"
#endif
#endif
...
uint32_t CFW_EmodSetRfB1AMPR(uint8_t nMode);
...
//2025-02-24 Add Start, ZhengYisong, MTC1014-344, add FrameNumber api.
#if (defined(CONFIG_FIBOCOM_BASE) && defined(CONFIG_CHIP_8910_NOR4M_PSRAM8M))
uint32_t CFW_EmodGetFrameNumber(uint16_t *pSysFrameNumber, uint16_t *pSubFrameNumber, CFW_SIM_ID nSimId);
//2025-05-09 Add Start, ZhengYisong, MTC0803-548, add GBRJTSimAttach api.
uint32_t CFW_EmodSetOptGBRJTSimAttach(uint8_t bEnable, CFW_SIM_ID nSimId);
//2025-05-09 Add End, ZhengYisong, MTC0803-548.
#endif
//2025-02-24 Add End, ZhengYisong, MTC1014-344.
...
typedef struct
{
    ...
    uint8_t reselPriority; /*小区重选优先级，范�?~7，无效�?xFF�*/
    uint8_t threshXLow;    /*低优先级小区重选门限threshX-Low，范�?~62.*/
    uint8_t threshXHigh;   /*高优先级小区重选门限threshX-High，范�?~62.*/
    uint8_t nccPermit;     /* NCC PERMIT，范�?~255.*/
    uint8_t reserver[3];
...
typedef struct
{
    ...
    cfw_lteCellAccessRelatedInfo_t cellAccRelInfo;
    cfw_lteCellReselInfo_t cellReselInfo; /* 小区的重选相关信�*/
} cfw_lteNcell_t_V2;
...
typedef struct
{
    ...
    uint16_t threshServingLow; /*服务小区低优先级重选门限threshServingLow；范�?~62�*/
    uint16_t sNonIntraSearch;  /*服务小区异频测量门限s-NonIntraSearch；范�?~62�*/
    uint16_t sIntraSearch;     /*服务小区同频测量门限s-IntraSearch；范�?~62�*/
...
uint32_t CFW_GetApnInfo(CFW_APNS_UNAME_UPWD *apn, uint8_t plmn[7]);
...
typedef struct T_CELL_STATS_RPT_TAG
{
    ...
    int8_t offset;        /*NB-IoT channel number?篓陇??篓庐篓虏EARFCN娄士??篓掳??隆杞柯郝μ�??娄士D篓庐D-0.5隆杞縜篓�?隆陇?隆锟铰¤娇???篓潞娄士娄食?2隆a?a隆饪★拷篓垄?娄士?娄士隆鑻�?                           篓篓??娄獭�??隆膦÷�?3篓娄??篓潞娄士娄台潞隆锟铰癮3y篓掳?2隆杩￠瓖
...
uint32_t CFW_SetRoaming(uint32_t roam, CFW_SIM_ID nSimID);
uint32_t CFW_GetRoaming(uint32_t *roam, CFW_SIM_ID nSimID);
...
uint32_t CFW_SetPsmT3412Len(uint32_t time, CFW_SIM_ID nSimID);
...
uint32_t CFW_SimCloseStatusTimer(uint8_t bClose, CFW_SIM_ID nSimId);
...
uint32_t CFW_GprsSetDataServiceHPrior(uint8_t nEnable, CFW_SIM_ID nSimID);

#if defined(CONFIG_SOC_8850) && defined(CFW_VOLTE_SUPPORT)
uint32_t CFW_SupportSrvccInLteOnly(uint8_t nFlag, CFW_SIM_ID nSimID);
#endif
...
//modify by fibocom 20240322 ,merge patch SPCSS01316151,just support in stable P7 ,need sync to main after 23.50.5 ,begin
uint32_t CFW_GprsSetT3247MaxTimes(uint8_t nEnable, CFW_SIM_ID nSimID);//nEnable:1 Maxtime is 5; 0 closed
//modify by  fibocom 20240322 ,end

//modify by fibocom 20240410 merge patch SPCSS01323863 BEGIN
typedef struct
{
    uint8_t nShortDrx;
    uint8_t nLongDrx;
...
typedef struct
{
    CFW_DRX_INDICATOR_T drxIndicator_fdd;
    CFW_DRX_INDICATOR_T drxIndicator_tdd;
} CFW_FEATURE_GROUP_INDICATOR_T;

uint32_t CFW_EmodSetFGI(CFW_FEATURE_GROUP_INDICATOR_T sFGI, CFW_SIM_ID nSimId);
uint32_t CFW_EmodGetFGI(CFW_FEATURE_GROUP_INDICATOR_T *pFGI, CFW_SIM_ID nSimId);
//modify by fibocom 20240410 END
#define SAT_PLI_GSM_NMR 0x00      // 2G
#define SAT_PLI_EUTRAN_INTRA 0x05 // LTE
#define SAT_PLI_EUTRAN_INTER 0x06 // LTE

#define SAT_PLI_UTRAN_INTRA 0x11               // 3G
#define SAT_PLI_UTRAN_INTER 0x12               // 3G
typedef struct _CFW_SAT_PROVIDE_LOCAL_INFO_RSP //for proactive command 0x21
{
    uint8_t nComID;
    uint8_t nComQualifier;
    uint8_t nType;
} CFW_SAT_PROVIDE_LOCAL_INFO_RSP;
uint32_t CFW_GetNwMeasureResult(uint8_t nType, uint16_t nUTI, CFW_SIM_ID nSimID);
bool CFW_SatGetProvideLocalInfoFMQualifier(uint8_t *FMQ, CFW_SIM_ID nSimID);
uint32_t CFW_SatSteeringRoaming(uint16_t nUTI, CFW_SIM_ID nSimID);

typedef enum _CFW_ERRC_CONN_ERROR_TYPE
{
    //CR8130_D_PS_C8320_NAS_V0.1.0_0027_IBUG8110
    /*for LTE used  NETWORK CAUSE*/
    CFW_ERRC_NORMAL_RELEASE,
    CFW_LOADBALANCING_TAU_REQUIRED,
    CFW_ERRC_CONNECTION_SETUP_FAILURE,
    CFW_ACCESS_CONTROL_BARRED,
    CFW_ERRC_INITAL_MSG_FAILURE,
    CFW_ERRC_UPLINK_MSG_FAILURE,
    CFW_ERRC_REESTABLISHMENT_FAILURE,
    CFW_CS_FALLBACKHIGHPRIORITY_V1020,
    /*add for res or redirection to td or GSM*/
    /*LOCAL CAUSE*/
    CFW_INACTIVE_LTE,
    CFW_REDIRECTION_RELEASE,
    CFW_ERRC_DISCARD_BECAUSE_STATE_ERROR,
    CFW_ACCESS_CONTROL_BARRED_CSFB,
    CFW_ACCESS_CONTROL_BARRED_MOCALL_AND_CSFB,
    CFW_ACCESS_CONTROL_BARRED_EAB,

    //added by ylm
    CFW_ERRC_NBIOT_RADIO_LINK_FAILURE,
    CFW_LTE_LOCAL_RELEASE,

    CFW_LTE_ELEC_FENCE_CELL_RELEASE, /*1570268, 20210407. Electronic fence cell. */
    /*1613367, 20210531. TAU fails without tau rej, but redirect to another lte freq in share-plmn enviroment.*/
    CFW_LTE_INTRA_REDIRECTION_RELEASE,
} CFW_ERRC_CONN_ERROR_TYPE;
uint32_t CFW_SimReadLargePreferPLMN(uint16_t nUTI, CFW_SIM_ID nSimID);
uint32_t CFW_FreeDynamicMemory(void *mem_address);

//2025-05-30 Add Start, ZhengYisong, MTC1034-382, patch from SPCSS01500966.
#ifdef CONFIG_MG661_JP_29_20_ZETIAN
typedef struct
{
    //phy comm
    int16_t avgRSRP;    //dbm
    int16_t avgSinr;    //db (-20~40)
    uint16_t transMode; //(1-10)

    //phy dl
    uint16_t dlBler;         //%
    uint16_t dlNewGrantBler; //%
    uint32_t dlPhyTpt;       //bps
    uint16_t dlGrantNum;
    uint16_t dlAvgHarqNum;
    uint16_t dlAvgPRB; // (0-100)
    uint16_t dlAvgMCS; //(0-28)
    uint16_t dlAvgTBS; // (0-1287) byte
    uint16_t dlAvgCqi;

    //phy ul
    uint16_t ulBler;
    uint16_t ulNewGrantBler; //%
    uint32_t ulPhyTpt;       //bps
    uint16_t ulGrantNum;
    uint16_t ulAvgHarqNum;
    uint16_t ulAvgPRB; // (0-100)
    uint16_t ulAvgMCS; //(0-28)
    uint16_t ulAvgTBS; // (0-654) byte
    int16_t txPower;   // dbm
} CFW_NET_SCHEDULE_PHY_INFO_T;

typedef struct
{
    //mac
    uint64_t macUlBytes;    //ulMacRate   byte
    uint64_t macUlPadBytes; //txPhyRate - ulMacRate  byte
    uint64_t macDlBytes;    //dlMacRate  byte
    uint64_t macDlPadBytes; //rxPhyRate - dlMacRate  byte
    uint32_t macUlTpt;      //ulMacRate * 8 * 1000 / ssfNum  bps
    uint32_t macDlTpt;      //dlMacRate * 8 * 1000 / ssfNum  bps

    //rlc
    uint64_t rlcUlPduBytes;  //ulRlcRate  byte
    uint32_t rlcUlRetxBytes; //rlcStatic [L2_WB_MAX_DRB_NUM] .ulAmDataPduRetrByte  byte
    uint64_t rlcDlPduBytes;  //dlRlcRate  byte
    uint32_t rlcUlTpt;       //ulRlcRate * 8 * 1000 / ssfNum  bps
    uint32_t rlcDlTpt;       //dlRlcRate * 8 * 1000 / ssfNum  bps

    //pdcp
    uint64_t pdcpUlPduBytes;     //ulPdcpRate  byte
    uint64_t pdcpDlPduBytes;     //dlPdcpRate  byte
    uint32_t pdcpUlDiscardBytes; //   byte
    uint32_t pdcpUlTpt;          //ulPdcpRate * 8 * 1000 / ssfNum  bps
    uint32_t pdcpDlTpt;          //dlPdcpRate * 8 * 1000 / ssfNum  bps
} CFW_NET_SCHEDULE_INFO_L2_T;

typedef struct
{
    CFW_NET_SCHEDULE_PHY_INFO_T phyScheInfo;
    CFW_NET_SCHEDULE_INFO_L2_T l2ScheInfo;
} CFW_NET_SCHEDULE_INFO;

uint32_t CFW_NwGetNetScheInfo(uint16_t nUTI, CFW_SIM_ID nSimID);
#endif
//2025-05-30 Add End, ZhengYisong, MTC1034-382.
