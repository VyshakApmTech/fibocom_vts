...
#if defined(CONFIG_LOCK_PLMN_SUPPORT)
    PB_OPT_ENC_ASSIGN(p->lockedPlmn.nStatus, nLockedPlumStatus);
    PB_OPT_ENC_ASSIGN(p->lockedPlmn.plmnNum, nLockedPlmnNum);
    PB_ENC_STRING(p->lockedPlmn.plmnList, lockedPlmnList);
#endif
...
#if defined(CONFIG_LOCK_PLMN_SUPPORT)
    PB_DEC_STRING(p->lockedPlmn.plmnList, lockedPlmnList);
#endif
...
#if defined(CONFIG_LOCK_PLMN_SUPPORT)
    PB_OPT_DEC_ASSIGN(p->lockedPlmn.nStatus, nLockedPlumStatus);
    PB_OPT_DEC_ASSIGN(p->lockedPlmn.plmnNum, nLockedPlmnNum);
#endif
    free(pbs);
    return true;
}
