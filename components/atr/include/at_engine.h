...
struct atDevice
{
    ...
    /** get autolocked baud */
    uint32_t (*get_autolocked_baud)(atDevice_t *th);
    #endif
	void (*cmd_line_write)(atDevice_t *th, const void *data, size_t size);
    /** the dispatch */
    atDispatch_t *recv;
};
...
/**
 * @brief Output AT command to AT device
 *
 * When receive AT command, AT command engine will output AT command
 * to AT device.
 *
 * The receiving data from AT device must be AT command format, this
 * function can be called.
 *
 * @param th        AT device, must be valid
 * @param data      AT command data
 * @param size      AT command data size
 */
void atDeviceCmdLineWrite(atDevice_t *th, const void *data, size_t size);
...
#ifdef CONFIG_FIBOCOM_BASE
uint32_t atDeviceGetAutolockedBaud(atDevice_t *th);
#endif
/**
...
/**
 * @brief dispatch AT command data
 *
 * When the dispatch is for device, AT device receives AT command data.
 *
 * @param th        the dispatch pointer, must be valid
 * @param data      AT command data
 * @param size      AT command data size
 */
void atDispatchCmdLineWrite(atDispatch_t *th, const uint8_t *data, unsigned size);
...
/**
 * @brief AT command to AT command mode engine
 *
 * When the parent dispatch is not in command mode, nothing will be done.
 *
 * If we want to use this function, we need to config
 * CONFIG_ATR_SUPPORT_DEVICE_ECHO_CMDLINE, when it is enabled, it will
 * work.
 *
 * @param th        command mode engine, must be valid
 * @param data      AT command data
 * @param size      AT command data size
 */
void atCmdLineWrite(atCmdEngine_t *th, const void *data, size_t length);
...
#ifdef CONFIG_FIBOCOM_BASE
uint8_t fibo_at_get_sim(atCmdEngine_t *th);
#endif
...
#define atCmdGetSim(th) fibo_at_get_sim(th)
#else
...
void atEngineSetDeviceDlytoSleep(int time);
#endif
...
#ifdef CONFIG_FIBOCOM_BASE
bool IsUartDispatch(atDispatch_t *dispatch);
#endif

int atDispatchGetDlci(atDispatch_t *th);
