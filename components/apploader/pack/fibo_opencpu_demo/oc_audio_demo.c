...
static void StreamPlayTask(void *ctx)
{
    ...
    vfs_close(fd);
    fibo_audio_mem_start(AUSTREAM_FORMAT_MP3, Testbuff, filesize, Callplayend);

    fibo_audio_stream_start(AUSTREAM_FORMAT_PCM,buff,&Read,&Write,(filesize+128), 20);
...
void audio_volume_test(void)
{
    //mode:AUDIO_VOICE_VOLUME(level:0-4)，AUDIO_PLAY_VOLUME(level:0-7)，AUDIO_TONE_VOLUME
    INT32 volume_level = 0;
    for (int level = 0; level <= 4; level++)
    {
        fibo_set_volume(AUDIO_TONE_VOLUME,level);
        OSI_LOGI(0, "fibo_set_volume level = %d", level);
        fibo_taskSleep(1000);
        volume_level = fibo_get_volume(AUDIO_TONE_VOLUME);
        OSI_LOGI(0, "fibo_get_volume level = %d", volume_level);
    }
#if 0
    for (int level = 0; level <= 7; level++)
    {
        fibo_set_volume(AUDIO_VOICE_VOLUME,level);
        OSI_LOGI(0, "fibo_set_volume level = %d", level);
        fibo_taskSleep(1000);
        volume_level = fibo_get_volume(AUDIO_VOICE_VOLUME);
        OSI_LOGI(0, "fibo_get_volume level = %d", volume_level);
    }
#endif
}

void audio_volume_external_test(void)
{
    //mode:AUDIO_VOICE_VOLUME(level:0-4)，AUDIO_PLAY_VOLUME(level:0-7)，AUDIO_TONE_VOLUME
    INT32 volume_external_level = 0;
    for (int level = 0; level <= 4; level++)
    {
        fibo_set_volume_external(AUDIO_TONE_VOLUME,level);
        OSI_LOGI(0, "fibo_set_volume level = %d", level);
        fibo_taskSleep(1000);
        volume_level = fibo_get_volume_external(AUDIO_TONE_VOLUME);
        OSI_LOGI(0, "fibo_get_volume level = %d", volume_external_level);
    }
#if 0
    for (int level = 0; level <= 7; level++)
    {
        fibo_set_volume_external(AUDIO_VOICE_VOLUME,level);
        OSI_LOGI(0, "fibo_set_volume_external level = %d", level);
        fibo_taskSleep(1000);
        volume_level = fibo_get_volume_external(AUDIO_VOICE_VOLUME);
        OSI_LOGI(0, "fibo_get_volume_external level = %d", volume_external_level);
    }
#endif
}
