...
bool fibo_app_valid_check(char *data, unsigned int len)
{
    ...
    const appImageHeader_t *header = (const appImageHeader_t *)data;
#else
...
//simage sig check
    ...
    if (data == NULL || len <= 0 || \
        ...
        {
            OSI_LOGE(0, "[FOTA]header info check failed");
            goto bail;
        }
    ...
    if (!OSI_REGION_INSIDE(uintptr_t, flashimg_flash_start, header->image_size,
                           flashimg_flash_start, flashimg_flash_size))
        {
            OSI_LOGE(0, "[FOTA]flashimg_flash check failed");
            goto bail;
        }
    ...
    if ((MAJOR(header->stub_version) != MAJOR(gCoreExportVersion)) || \
        ...
        {
            OSI_LOGE(0, "[FOTA]header version check failed");
            goto bail;
        }
    ...
    if (drvSecureBootEnable())
    {
        OSI_LOGI(0x10007cbe, "ota app check signature");
...
/**
* @brief Verify whole app package by path
*
* @param path whole app package directory
* @return  ture: verify success
           false:verify failed
 */
bool fibo_ota_v2_whole_app_check(const char *path)
{
    unsigned  zero = 0;
    char *seg_data = NULL;
    int seg_len = 4*1024;
    ...
    seg_fd = vfs_open(path, O_RDONLY);
    if (-1 == seg_fd){
        OSI_LOGE(0, "[OTA_V2]open file fail!");
        return false;
    }

    seg_data = malloc(seg_len);
    if (NULL == seg_data){
        OSI_LOGE(0, "[OTA_V2]malloc fail");
        return false;
    }

    memset(seg_data, 0, seg_len);
    read_len = seg_len;
    rsize = vfs_read(seg_fd, seg_data, read_len);
    if(rsize < 0){
        OSI_LOGE(0, "[OTA_V2]read fail! rsize = %d", rsize);
        goto bail;
    }

    const appImageHeader_t *header = (const appImageHeader_t *)(seg_data + APPIMG_OFFSET);
//simage sign check
#ifdef CONFIG_SOC_8850
    simageHeader_t *sheader = (simageHeader_t *)seg_data;
    if (simageHeaderValid(sheader, flashimg_flash_size))
    {
#if defined(CONFIG_APPIMG_SIGCHECK_ENABLE)
        OSI_LOGI(0x10007cbe, "check app signature");
        if (!appSimageCheck((const void *)sheader))
        {
            OSI_LOGE(0x10007cbf, "whole_app.pack signature check failed");
            goto bail;
        }
#endif
    }
    ...
    {
        OSI_LOGE(0, "whole_app.pack format is incorrect");
        goto bail;
    }
    ...
    OSI_LOGI(0, "[OTA_V2]whole_app.pack magic/0x%x size/%d crc/0x%x enter/0x%x exit/0x%x stub/0x%x",
             header->magic, header->image_size, header->image_crc,
             header->enter_function, header->exit_function,
             header->stub_version);

    image_crc = header->image_crc; //header will change with the change of seg_data, so save image_crc and image_size
    image_size = header->image_size;
    crc = crc32Update(crc, header, 8);
    crc = crc32Update(crc, &zero, 4);

    while(seg_offset < (image_size - 12)) {
        memset(seg_data, 0, seg_len);
        vfs_lseek(seg_fd, seg_offset + APPIMG_OFFSET + 12, SEEK_SET);
        read_len = (image_size - 12 - seg_offset) < seg_len ? (image_size - 12 - seg_offset) : seg_len;
        rsize = vfs_read(seg_fd, seg_data, read_len);
        if(rsize < 0){
            OSI_LOGE(0, "[OTA_V2]read fail! rsize = %d", rsize);
            goto bail;
        ...
        crc = crc32Update(crc, seg_data, rsize);
        seg_offset += rsize;
    }

    if (crc != image_crc) {
        OSI_LOGE(0, "[OTA_V2]Invalid whole_app.pack crc/0x%x calc/0x%x", image_crc, crc);
        goto bail;
    }

//success:
    OSI_LOGI(0, "[OTA_V2]whole_app.pack check successful!");
    if(seg_data) {
        free(seg_data);
        seg_data = NULL;
    }
...
bail:
    OSI_LOGE(0, "[OTA_V2]whole_app.pack check error!");
    if(seg_data) {
        free(seg_data);
        seg_data = NULL;
    }
