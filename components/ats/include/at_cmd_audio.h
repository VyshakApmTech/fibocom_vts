...
extern HAL_EXT_CODEC_T g_hal_ext_codec_cfg;
#endif
...
void ffw_audio_pocplay(uint32_t samplerate, uint32_t format, auPlayerCallback_t cb_ctx);
void ffw_audio_pocplay_stop(void);
void ffw_audio_pocrecord(uint32_t samplerate, uint32_t format, uint8_t *audio_buff,  uint32_t *Wp, uint32_t buffsize);
void ffw_audio_pocrecord_stop(void);
