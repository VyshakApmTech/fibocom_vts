...
#ifdef CONFIG_USB_ETHER_SUPPORT
    {&gSysnvUsbWorkMode, SYSNV_TYPE_INT, 0, 13, "usbmode",
...
void atCmdHandleGTUSBMODE(atCommand_t *cmd)
{
    ...
    uint8_t usbmaxnum=85;
    uint8_t list[13] ={2, 5, 3,DRV_8850USB_ELEC_ECM,DRV_8850USB_SERIALS_SPECIAL,DRV_USB_PRINTER,
                      DRV_USB_PRINTER_AND_SERIALS,DRV_USB_HID_ACM_SERIALS,DRV_8850USB_HIDKEYBOARD_AND_SERIALS,
                      DRV_8850USB_HIDBULK_AND_AT,DRV_8850USB_HIDBULK,DRV_8850USB_HIDBULK_AND_AT_AT2,DRV_8850USB_BULK_AND_AT};
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        if ( (!fibo_get_usb_hidacm_enable()) && (val==80) )
            RETURN_CME_ERR(cmd->engine, ERR_AT_CME_EXE_NOT_SURPORT);

        #ifndef CONFIG_FIBOCOM_HID_KEYBOARD_SUPPORT
        if ((val==81)||(val==82)||(val==83)||(val==84)||(val==85))
            RETURN_CME_ERR(cmd->engine, ERR_AT_CME_EXE_NOT_SURPORT);
        #endif
    ...
    else if (AT_CMD_TEST == cmd->type)
    {
        ...
        switch(flag)
        {
              ...
              /*MTC0789-59 begin wj add acm support*/
              case 0xC:
              snprintf(resp, 64, "+GTUSBMODE: (73-75,80)");
                      break;
              case 0xD:
              snprintf(resp, 64, "+GTUSBMODE: (73-76,80)");
                      break;
              case 0xE:
              snprintf(resp, 64, "+GTUSBMODE: (73-75,77,80)");
                      break;
              case 0xF:
              snprintf(resp, 64, "+GTUSBMODE: (73-77,80)");
                      break;
              /*MTC0789-59 end wj add acm support*/
        }
        atCmdRespInfoText(cmd->engine, resp);
...
void atCmdHandleGTUSBMODE(atCommand_t *cmd)
{
    ...
    uint8_t list[3] ={DRV_USB_SERIALS, DRV_USB_ECM_AND_SERIALS, DRV_USB_RNDIS_AND_SERIALS};

    const sysnvItem_t *item = prvNameGetItem(name);
    if (AT_CMD_SET == cmd->type)
    {
        int val = atParamIntInRange(cmd->params[0], 31, 33, &paramok);
        if (!paramok)
            RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_ERROR);
        val = list[val - 31];
        ITEM_INT_VAL(item->value) = val;
    ...
    {
        char resp[32];
        snprintf(resp, 32, "+GTUSBMODE: (31-33)");
