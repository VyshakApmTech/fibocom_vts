...
def readApn(argv):
    ...
    with open(args.input,"r",encoding="mac_roman") as fr:
        ...
        for line in reader:
            if not line[0]:
                continue
            if ((line[6] == '1') or (line[6] == '4') or (line[6] == '6') or (line[6] == '7')):
                continue
            if (len(line[6]) > 1): #skip the first row the table
                continue
            if len(line[5]) == 1:
                list1.append([int(line[4] + '0' + line[5]), int(line[6]), line[7], line[12], line[13]])
            else:
                list1.append([int(line[4] + line[5]), int(line[6]), line[7], line[12], line[13]])
        ...
        with open(args.output, 'w') as fw:
            ...
            fw.write('    const uint8_t accountType;\n')
            ...
            for i in range(len(list1)):
                fw.write('    {' + str(list1[i][0]) + ', ' + str(list1[i][1]) + ', "' + list1[i][2] + '", "' + list1[i][3] + '", "' + list1[i][4] + '"},\n')
            fw.write('};\n')
            fw.close()
