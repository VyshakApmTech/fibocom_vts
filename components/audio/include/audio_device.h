...
uint16_t audevGetVoiceVolume(void);
#else
 ...
 */
#if defined(CONFIG_FIBOCOM_BASE)
uint16_t audevGetPlayVolume(void);
#else
...
//add by fibocom
void audevGetVolume(uint8_t channel, uint8_t *voice, uint8_t *tone, uint8_t *midi);
bool audeSetVoice(uint8_t accy, uint8_t vol);
bool audeSetTone(uint8_t accy, uint8_t vol);
bool audeSetMidi(uint8_t accy, uint8_t vol);
...
extern int ffw_cdtmf_local_tone_play(uint8_t tone_id,uint32_t tone_duration);
extern int ffw_cdtmf_local_tone_stop(void);
#endif
