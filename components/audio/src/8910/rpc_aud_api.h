...
void aud_SetCalibVerifyFlag(UINT32 flag);
HAL_ERR_T hal_btVoiceStart(void);
HAL_ERR_T hal_btVoiceStop(void);
void aud_SetCloseDelayFlag(HAL_ERR_T  en);
HAL_ERR_T hal_zspVoiceDtmfStart(void);
HAL_ERR_T hal_zspVoiceDtmfStop(void);
void aud_CodecNvInit(void);
AUD_ERR_T aud_SetLoopbackGain(SND_ITF_T itf, AUD_CODEC_LOOPBACK_OUT_GAIN_T *outGain, AUD_INPUT_TYPE_T inputType, AUD_CODEC_LOOPBACK_IN_GAIN_T *inGain);
HAL_AIF_FREQ_T vois_GetSampleRate(void);
void aud_CodecSetExtFlag(UINT16 externalCodec);
AUD_ERR_T CSW_SetVqeCalibParamSwitch(INT16 nEnable, SND_APP_MODE_T nMode, AUD_ITF_T nItf, CALIB_AUD_RDA_VQE_CTRL_T nCtrl);
void VTSP_startVolteCodec(void);
void VTSP_stopVolteCodec(void);
UINT32 DSPExt_GetVoiceCoderType(void);
UINT32 aud_SetNewLoopTestFlag(UINT32 flag);
HAL_ERR_T DM_AudSetPlayAlertingTone(UINT8 bToneMode);
HAL_ERR_T DM_AudGetPlayAlertingTone(uint8_t *pToneMode);
void aud_PocNvInit(UINT16 PocInitFlag);
