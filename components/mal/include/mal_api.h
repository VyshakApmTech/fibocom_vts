 ...
 *      - SUCCESS
 */
MAL_SMS_MSG_INFO_T *mal_SmsGetMsgInfo(uint16_t nIndex, uint8_t nSimId);
...
#define MAL_SIM_IMSI_LEN 16
/** 
 * @brief (CIMI ExeCommand) 
 *
 * Description: this api is used to get imsi.
 *
 * @param imsi        save imsi
 * @param imsi_len  save imsi_len
 * @param nSim       SIM index, ignored in single SIM version
 * @return
 *      - 0 on success
 *      - error code on fail
 */
unsigned malGetSimProviderId(uint8_t *imsi, uint8_t *imsi_len, uint8_t nSim);
#ifdef CONFIG_FIBOCOM_BASE
int malGetCnum(uint8_t *num, uint8_t *len, uint8_t nSim);
int fibo_sim_channel_transmit(uint8_t channel_id, uint8_t *apdu, uint16_t apduLen,
                         uint8_t *resp, uint16_t *respLen, uint16_t timeout, uint8_t sim_id);

int fibo_sim_cchc_manage_channel(uint8_t command,uint8_t channel,uint8_t sim_id);
int fibo_sim_ccho_select_application(uint8_t *aid, uint8_t length, uint8_t channel,uint8_t simid);
int fibo_sim_ccho_manage_channel(uint8_t command,uint8_t channel, uint8_t *sim_channl,uint8_t sim_id);
