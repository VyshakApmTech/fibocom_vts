...
#ifdef CONFIG_FIBOCOM_BASE // reset pin maybe not used
bool g_is_rst_pin_unused = false;
#endif
...
static void prvCamIomuxCfg(camInputSensor_t type)
{
    ...
    case camSpi_In:
    {
        ...
        REGT_FIELD_CHANGE(hwp_iomux->spi_camera_si_0, REG_IOMUX_SPI_CAMERA_SI_0_T, func_sel, 0);
        REGT_FIELD_CHANGE(hwp_iomux->spi_camera_si_1, REG_IOMUX_SPI_CAMERA_SI_1_T, func_sel, 0);
        REGT_FIELD_CHANGE(hwp_iomux->spi_camera_sck, REG_IOMUX_SPI_CAMERA_SCK_T, func_sel, 0);
        #ifdef CONFIG_FIBOCOM_BASE // reset pin maybe not used
        if (g_is_rst_pin_unused == false) {
            REGT_FIELD_CHANGE(hwp_iomux->camera_rst_l, REG_IOMUX_CAMERA_RST_L_T, func_sel, 0);
        }
        #else
        REGT_FIELD_CHANGE(hwp_iomux->camera_rst_l, REG_IOMUX_CAMERA_RST_L_T, func_sel, 0);
        #endif
        REGT_FIELD_CHANGE(hwp_iomux->camera_pwdn, REG_IOMUX_CAMERA_PWDN_T, func_sel, 0);
        REGT_FIELD_CHANGE(hwp_iomux->camera_ref_clk, REG_IOMUX_CAMERA_REF_CLK_T, func_sel, 0);
        #ifndef CONFIG_FIBOCOM_BASE // select by customer
        REGT_FIELD_CHANGE(hwp_iomux->i2c_m2_scl, REG_IOMUX_I2C_M2_SCL_T, func_sel, 0);
        REGT_FIELD_CHANGE(hwp_iomux->i2c_m2_sda, REG_IOMUX_I2C_M2_SDA_T, func_sel, 0);
        #endif
    ...
    case camCsi_In:
    {
        ...
        REGT_FIELD_CHANGE(hwp_iomux->camera_pwdn, REG_IOMUX_CAMERA_PWDN_T, func_sel, 0);        // PWDN
        REGT_FIELD_CHANGE(hwp_iomux->camera_ref_clk, REG_IOMUX_CAMERA_REF_CLK_T, func_sel, 0);  // MCLK
        hwp_iomux->pad_camera_rst_l |= 0x400;       // CKP
        hwp_iomux->pad_spi_camera_sck |= 0x400;     // CKN
        hwp_iomux->pad_spi_camera_si_1 |= 0x400;    // D0P
        hwp_iomux->pad_spi_camera_si_0 |= 0x400;    // D0N
        #endif
#elif defined(CONFIG_SOC_8910)
        REG_IOMUX_PAD_CAMERA_RST_L_CFG_REG_T pad_camera_rst_l_cfg_reg = {0};
        REG_IOMUX_PAD_SPI_CAMERA_SCK_CFG_REG_T pad_spi_camera_sck_cfg_reg = {0};
...
{
#if defined(CONFIG_SOC_8850)
    REG_AP_CLK_CGM_CAMERA_REF_SEL_CFG_T cgm_camera_ref_sel;
    REG_AP_CLK_CGM_CAMERA_REF_DIV_CFG_T cgm_camera_ref_div;
    //clock src APLL500M
    cgm_camera_ref_sel.b.cgm_camera_ref_sel = CONFIG_CAM_MCLK_SRC_NUM;
    hwp_apClk->cgm_camera_ref_sel_cfg = cgm_camera_ref_sel.v;

#ifdef CONFIG_FIBOCOM_BASE
    cgm_camera_ref_div.b.cgm_camera_ref_div = (CONFIG_DEFAULT_CAM_MCLK_SRC_FREQ / (Mclk * 1000 * 1000) - 1);
    hwp_apClk->cgm_camera_ref_div_cfg = cgm_camera_ref_div.v;
#else
//camera Mclock 15M
    cgm_camera_ref_div.b.cgm_camera_ref_div = (CONFIG_DEFAULT_CAM_MCLK_SRC_FREQ / CONFIG_DEFAULT_CAM_MCLK_FREQ - 1);
    hwp_apClk->cgm_camera_ref_div_cfg = cgm_camera_ref_div.v;
#endif
    hwp_iomux->pad_camera_ref_clk = (hwp_iomux->pad_camera_ref_clk & ~IOMUX_PAD_CAMERA_REF_CLK_DRV(0xf)) | IOMUX_PAD_CAMERA_REF_CLK_DRV(0x1);
    REGT_FIELD_CHANGE(hwp_apApb->misc_cfg, REG_AP_APB_MISC_CFG_T, camera_refclk_out_en, 1);
    REGT_FIELD_CHANGE(hwp_iomux->spi_camera_si_0, REG_IOMUX_SPI_CAMERA_SI_0_T, func_sel, 0);
    REGT_FIELD_CHANGE(hwp_iomux->spi_camera_si_1, REG_IOMUX_SPI_CAMERA_SI_1_T, func_sel, 0);
    REGT_FIELD_CHANGE(hwp_iomux->spi_camera_sck, REG_IOMUX_SPI_CAMERA_SCK_T, func_sel, 0);
#ifdef CONFIG_FIBOCOM_BASE // reset pin maybe not used
    ...
    hwp_iomux->pad_debug_host_rx |= 0x400;
    hwp_iomux->pad_debug_host_clk |= 0x400;
    hwp_iomux->pad_camera_rst_l |= 0x400;
    hwp_iomux->pad_spi_camera_sck |= 0x400;
    hwp_iomux->pad_spi_camera_si_1 |= 0x400;
    hwp_iomux->pad_spi_camera_si_0 |= 0x400;
    cgm_camera_ref_sel.b.cgm_camera_ref_sel = CONFIG_CAM_MCLK_SRC_NUM;
    hwp_apClk->cgm_camera_ref_sel_cfg = cgm_camera_ref_sel.v;
    cgm_camera_ref_div.b.cgm_camera_ref_div = (CONFIG_DEFAULT_CAM_MCLK_SRC_FREQ / CONFIG_DEFAULT_CAM_MIPI_MCLK_FREQ - 1);
    hwp_apClk->cgm_camera_ref_div_cfg = cgm_camera_ref_div.v;
#endif
#endif
...
{
#if defined(CONFIG_SOC_8850)
    REGT_FIELD_CHANGE(hwp_apApb->misc_cfg, REG_AP_APB_MISC_CFG_T, camera_refclk_out_en, 0);
#elif defined(CONFIG_SOC_8910)
...
void drvCampRegInit(sensorInfo_t *config)
{
...
#if defined(CONFIG_SOC_8850)
    REG_AP_CLK_CGM_CAMERA_PIX_SEL_CFG_T cgm_camera_pix_sel_cfg;
    REG_AP_CLK_CGM_CAMERA_PIX_DIV_CFG_T cgm_camera_pix_div_cfg;
    REG_AP_CLK_CGM_CAMERA_CSI_SEL_CFG_T cgm_camera_csi_sel_cfg;
    REG_AP_CLK_CGM_CAMERA_CSI_DIV_CFG_T cgm_camera_csi_div_cfg;
#elif defined(CONFIG_SOC_8910)
    REG_SYS_CTRL_SEL_CLOCK_T sel_clock;
...
#if defined(CONFIG_SOC_8850)
    cgm_camera_pix_sel_cfg.v = hwp_apClk->cgm_camera_pix_sel_cfg;
    cgm_camera_pix_div_cfg.v = hwp_apClk->cgm_camera_pix_div_cfg;
    cgm_camera_csi_sel_cfg.v = hwp_apClk->cgm_camera_csi_sel_cfg;
    cgm_camera_csi_div_cfg.v = hwp_apClk->cgm_camera_csi_div_cfg;
#elif defined(CONFIG_SOC_8910)
    sel_clock.v = hwp_sysCtrl->sel_clock;
...
#if defined(CONFIG_SOC_8850)
    cgm_camera_pix_sel_cfg.b.cgm_camera_pix_sel = CONFIG_CAM_PCLK_SRC_NUM;
    hwp_apClk->cgm_camera_pix_sel_cfg = cgm_camera_pix_sel_cfg.v;
    cgm_camera_pix_div_cfg.b.cgm_camera_pix_div = (CONFIG_DEFAULT_CAM_PCLK_SRC_FREQ / CONFIG_DEFAULT_CAM_PCLK_FREQ - 1);
    hwp_apClk->cgm_camera_pix_div_cfg = cgm_camera_pix_div_cfg.v;
    cgm_camera_csi_sel_cfg.b.cgm_camera_csi_sel = CONFIG_CAM_CCLK_SRC_NUM;
    hwp_apClk->cgm_camera_csi_sel_cfg = cgm_camera_csi_sel_cfg.v;
    cgm_camera_csi_div_cfg.b.cgm_camera_csi_div = (CONFIG_DEFAULT_CAM_CCLK_SRC_FREQ / CONFIG_DEFAULT_CAM_CCLK_FREQ - 1);
    hwp_apClk->cgm_camera_csi_div_cfg = cgm_camera_csi_div_cfg.v;
#elif defined(CONFIG_SOC_8910)
    ...
    if (config->sensorType == camSpi_In)
    {
        if (config->camSpiMode != SPI_MODE_NO)
        {
            ...
            #ifdef CONFIG_FIBOCOM_BASE
            if (config->image_format == CAM_FORMAT_ONLY_Y) {
                reg0Cfg.b.block_num_per_line = config->spi_pixels_per_line >> 2;
            } else {
            ...
            REGT_FIELD_CHANGE(hwp_camera->spi_camera_reg1, REG_CAMERA_SPI_CAMERA_REG1_T,
                              camera_clk_div_num, config->spi_ctrl_clk_div,
                              sck_ddr_en, config->ddr_en);
            ...
            case SPI_MODE_MASTER2_2:
                ...
                #ifdef CONFIG_FIBOCOM_BASE
                if (config->image_format == CAM_FORMAT_ONLY_Y) {
                    reg4Cfg.b.block_num_per_packet = config->spi_pixels_per_line >> 2;
                } else {
                    reg4Cfg.b.block_num_per_packet = config->spi_pixels_per_line >> 1;
                }
                #else
...
bool drvCamGc2145SetRst(bool rstActiveH)
{
    ...
    drvGpioConfig_t cfg = {
        ...
        .out_level = rstActiveH,
    };

    REGT_FIELD_CHANGE(hwp_iomux->gpio_4, REG_IOMUX_GPIO_4_T, func_sel, 0);
    drvGpio_t *d = drvGpioOpen(4, &cfg, NULL, NULL);
    if (d == NULL)
        return false;

    drvGpioWrite(d, rstActiveH);
    return true;
}
#endif
