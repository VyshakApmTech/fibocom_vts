...
#ifndef CONFIG_FIBOCOM_8910_BASE
...
/**
 * Flash clk shift for RX sensitivity
 */
#define FLASH1_FREQ_REQ_100M (100)
#define FLASH1_FREQ_REQ_83M (83)

#define FLASH1_FREQ_100M_SETTING (0xc)
#define FLASH1_FREQ_83M_SETTING (0xb)
#define FLASH1_SPI_BUSY_FLAG (0x1)
uint32_t FlashFreqVal = 0;
osiTimer_t *FlashClkChangeTimer = NULL;

FLASHRAM_API void drvChangeFlashClk(void)
{
    osiTimerStop(FlashClkChangeTimer);

    uint32_t critical = osiEnterCritical();
    uint32_t spi_status = hwp_spiFlash->spi_status;
    if ((spi_status & FLASH1_SPI_BUSY_FLAG) == 0)
    {
        if (FlashFreqVal == FLASH1_FREQ_REQ_100M)
        {
            //500M/2.5/2 = 100M
            //REGT_FIELD_CHANGE(hwp_spiFlash->spi_config, REG_SPI_FLASH_SPI_CONFIG_T, clk_divider, 2);
            REGT_FIELD_CHANGE(hwp_apApb->cfg_clk_spiflash1, REG_AP_APB_CFG_CLK_SPIFLASH1_T, spiflash1_freq, FLASH1_FREQ_100M_SETTING);
        }
        else if (FlashFreqVal == FLASH1_FREQ_REQ_83M)
        {
            //500M/3/2 = 83M
            //REGT_FIELD_CHANGE(hwp_spiFlash->spi_config, REG_SPI_FLASH_SPI_CONFIG_T, clk_divider, 2);
            REGT_FIELD_CHANGE(hwp_apApb->cfg_clk_spiflash1, REG_AP_APB_CFG_CLK_SPIFLASH1_T, spiflash1_freq, FLASH1_FREQ_83M_SETTING);
        }
        else
            OSI_LOGI(0, "spi_flash change fail!");
    }
    else
    {
        osiTimerStart(FlashClkChangeTimer, 40);
    }
    osiExitCritical(critical);
}
void drvSpiFlashClkShift(uint32_t flash_freq)
{
    if (FlashFreqVal == flash_freq)
        return;

    FlashFreqVal = flash_freq;

    OSI_LOGI(0, "spi_flash change to %d M!", FlashFreqVal);

    if (FlashClkChangeTimer == NULL)
    {
        FlashClkChangeTimer = osiTimerCreate(NULL, (osiCallback_t)drvChangeFlashClk, NULL);
    }
    drvChangeFlashClk();
}
#endif
