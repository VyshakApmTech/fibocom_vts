...
#define LCD_DRV_ID_ST7789v1       0X858552
...
static uint32_t _stReadIdv1(void);
#endif
static uint32_t _stReadIdv2(void);
...
#if defined(CONFIG_L610_CN_00_90_QIE) || defined(CONFIG_L610_CN_62_39_NONVOLTE_QIE)
...
)
{
    ...
    if ((right < left) || (bottom < top))
    {
        ...
        OSI_ASSERT(false, "lcd[st7789v2]: in _stInvalidateRect error"); /*assert verified*/
    }
...
const lcdSpec_t g_lcd_st7789v2 =
    {
        LCD_DRV_ID_ST7789v2,
        LCD_WIDTH,
        LCD_HEIGHT,
...
static uint32_t _stReadIdv2(void)
{
    ...
    _lcdDelayMs(10);
#endif
    st_farmk = g_lcd_st7789v2.is_use_fmark;
    st_line_mode = g_lcd_st7789v2.bus_mode;

    halGoudaReadConfig(st_line_mode, g_lcd_st7789v2.is_use_fmark, g_lcd_st7789v2.fmark_delay);
    _lcdDelayMs(140);
    ...
    if (LCD_DRV_ID_ST7789v2 == ret_id)
    {
        OSI_LOGI(0, "lcd[st7789v2]:  st is using !");
    }
...
#ifdef CONFIG_FIBOCOM_LCD_ST7789_4LINE
static const lcdOperations_t st7789v1Operations =
    {
        _stInit,
        _stSleepIn,
        _stEsdCheck,
...
const lcdSpec_t g_lcd_st7789v1 =
    {
        LCD_DRV_ID_ST7789v1,
        LCD_WIDTH,
        LCD_HEIGHT,
        HAL_GOUDA_SPI_LINE_4,
        LCD_CTRL_SPI,
        (lcdOperations_t *)&st7789v1Operations,
        false,
        0x2a000,
        50000000,
};
...
static uint32_t _stReadIdv1(void)
{
    ...
    HAL_ERR_T r_err;
    uint8_t id[4] = {0};
    OSI_LOGI(0, "lcd[st7789v1]:  st7789v1   _stReadId \n");

#ifdef CONFIG_FIBOCOM_BASE
#ifdef CONFIG_MC665_CN_36_90_QDTIANYAN // FIBOCOM: not recommend configurate power here
    halPmuSwitchPower(HAL_POWER_LCD, true, true);
    ...
    st_farmk = g_lcd_st7789v1.is_use_fmark;
    st_line_mode = g_lcd_st7789v1.bus_mode;

    halGoudaReadConfig(st_line_mode, g_lcd_st7789v1.is_use_fmark, g_lcd_st7789v1.fmark_delay);
    _lcdDelayMs(140);
    //hwp_analogReg->pad_spi_lcd_cfg2 |= ANALOG_REG_PAD_SPI_LCD_SIO_SPU;
    _lcdDelayMs(10);
    r_err = halGoudaReadData(0x04, id, 4);
    //hwp_analogReg->pad_spi_lcd_cfg2 =  hwp_analogReg->pad_spi_lcd_cfg2&(~ANALOG_REG_PAD_SPI_LCD_SIO_SPU);
    _lcdDelayMs(10);
    OSI_LOGI(0, "lcd[st7789v1]:  _stReadId LCM: 0x%0x, 0x%x, 0x%0x (read return : %d)\n", id[3], id[2], id[1], r_err);

    ret_id = ((id[3] << 16) | (id[2] << 8) | id[1]);

    if (LCD_DRV_ID_ST7789v2 == ret_id)
    {
        OSI_LOGI(0, "lcd[st7789v1]:  st is using !");
    }
    OSI_LOGI(0, "lcd[st7789v1]: st7789v1 ReadId:0x%x \n",ret_id);
#ifndef CONFIG_FIBOCOM_BASE
    halGoudaClose();
    halPmuSwitchPower(HAL_POWER_LCD, false, false);
#endif
