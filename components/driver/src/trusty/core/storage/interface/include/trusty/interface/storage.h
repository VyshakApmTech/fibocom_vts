...
/*
 * Storage port names
 * @STORAGE_CLIENT_TO_PORT:     Note that Data is written to the /data in plaintext
 * @STORAGE_CLIENT_TN_PORT:     Port used by clients that require tamper and
 *                              rollback detection. Note that data is written to prodnv
 * @STORAGE_CLIENT_TE_PORT:     Port used by clients that require tamper and
 *                              rollback detection. Note that data is written to emmc
 ...
 */
#define STORAGE_CLIENT_TO_PORT "com.android.trusty.storage.client.to"
#define STORAGE_CLIENT_TN_PORT "com.android.trusty.storage.client.tn"
#define STORAGE_CLIENT_TE_PORT "com.android.trusty.storage.client.te"
...
enum storage_cmd
{
    ...
    STORAGE_RPMB_PROXY_CONNECT = 12 << STORAGE_REQ_SHIFT,
    STORAGE_NS_PROXY_CONNECT = 13 << STORAGE_REQ_SHIFT,
    STORAGE_EMMC_PROXY_CONNECT = 14 << STORAGE_REQ_SHIFT,
    STORAGE_EMMC_READ = 15 << STORAGE_REQ_SHIFT,
    STORAGE_EMMC_WRITE = 16 << STORAGE_REQ_SHIFT,
    STORAGE_PRODNV_PROXY_CONNECT = 17 << STORAGE_REQ_SHIFT,
    STORAGE_PLAINTEXT_PROXY_CONNECT = 18 << STORAGE_REQ_SHIFT,
    STORAGE_FILE_DEL_NO_PERSIST = 19 << STORAGE_REQ_SHIFT,
};
...
enum storage_err
{
    ...
    STORAGE_ERR_NOT_PROGRAM_RPMB_KEY = 8,
    STORAGE_ERR_RPMB_KEY_DIFFERENT = 9,
};
 ...
 *                                      file does not exist.
 * @STORAGE_FILE_OPEN_CREATE_NOPERSIST: mark the file, delete when factory is restored
 *                                      Only meaningful if used in combination with STORAGE_FILE_OPEN_CREATE.
...
/**
 * struct storage_emmc_proxy_connect_req - request format for STORAGE_EMMC_PROXY_CONNECT
 * @emmc_size:        size in bytes of emmc write region
 *
 * Only used in proxy<->server interface.
 */
struct storage_emmc_proxy_connect_req
{
    uint32_t emmc_size;
};

/**
 * struct storage_prodnv_proxy_connect_req - request format for STORAGE_PRODNV_PROXY_CONNECT
 * @emmc_size:        size in bytes of prodnv partition
 *
 * Only used in proxy<->server interface.
 */
struct storage_prodnv_proxy_connect_req
{
    uint32_t prodnv_size;
};
