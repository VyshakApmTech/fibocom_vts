...
void drvCamFreeFrameBuffer(void)
{
    OSI_LOGI(0, "cam: drvCamFreeFrameBuffer");
    for (uint8_t i = 0; i < ImageSensor.framebuffer_count; i++)
    {
        if (ImageSensor.pFramebuffer[i] != NULL)
        {
...
#ifdef CONFIG_FIBOCOM_BASE
    if (!osiSemaphoreTryAcquire(ImageSensor.pSensorInfo->cam_sem_preview, 5000))
#else
...
#endif
{
    ...
    if (ImageSensor.pSensorOpsApi != NULL)
    {
        ...
        pCamDevice->image_format = ImageSensor.pSensorInfo->image_format;
...
// #endif    
    ...
    if (ImageSensor.pSensorOpsApi != NULL)
    {
        ...
        } else if (ImageSensor.pSensorInfo->image_format == CAM_FORMAT_RAW10) {
            ImageSensor.nPixcels = ImageSensor.pSensorInfo->spi_pixels_per_line * ImageSensor.pSensorInfo->spi_pixels_per_column * 10 / 8;
        } else {
