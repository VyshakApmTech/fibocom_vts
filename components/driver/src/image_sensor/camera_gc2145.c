...
/**
 * @brief FIBOCOM example (defalut off): camera GC2145 @1600*1200, RAW10
 */
// #define _FIBOCOM_EXAMPLE_CAM_GC2145_1600X1200_RAW10

/**
 * @brief FIBOCOM example (defalut off): camera GC2145 @1600*1200, YUV422
 */
// #define _FIBOCOM_EXAMPLE_CAM_GC2145_1600X1200_YUV422
#endif
...
static const cameraReg_t RG_InitVgaMIPI[] =
    {
#if defined(CONFIG_FIBOCOM_BASE) && defined(_FIBOCOM_EXAMPLE_CAM_GC2145_1600X1200_RAW10)
    /*
    * @author Ethan Guo (Guo Junhuang) (ethan.guo@fibocom.com)
    * @date 2025-06-28
    * @brief Format: RAW10
             Resolution ratio: 1600*1200
             framerate: 4pfs, based on 500MHz CSI clock, 24MHz MCLK and the sensor configurations below.
    * @note
            1. Make sure that RAM has enough RAM space for framebuffer to store data, based on its format and resolution ratio. 
                In this case (1600*1200*10/8) bytes are required at least.
            2. Make sure that camera CSI clock frequency is sufficiently matched to the configuration of the sensor. 
                In this case you can set CSI clock as 500MHz, by adding 'CONFIG_DEFAULT_CAM_CCLK_FREQ=500000000' in file 'target.config'.
            3. The frame rate may not reach exactly what you desire. Users need to debug and determine the optimal frame rate by themselves,
...
#elif defined(CONFIG_FIBOCOM_BASE) && defined(_FIBOCOM_EXAMPLE_CAM_GC2145_1600X1200_YUV422)
    /*
    * @author Ethan Guo (Guo Junhuang) (ethan.guo@fibocom.com)
    * @date 2025-06-28
    * @brief Format: YUV422(YUYV)
             Resolution ratio: 1600*1200
             framerate: 4pfs, based on 500MHz CSI clock, 24MHz MCLK and the sensor configurations below.
    * @note
            1. Make sure that RAM has enough RAM space for framebuffer to store data, based on its format and resolution ratio. 
                In this case (1600*1200*2) bytes are required at least.
            2. Make sure that camera CSI clock frequency is sufficiently matched to the configuration of the sensor. 
                In this case you can set CSI clock as 500MHz, by adding 'CONFIG_DEFAULT_CAM_CCLK_FREQ=500000000' in file 'target.config'.
            3. The frame rate may not reach exactly what you desire. Users need to debug and determine the optimal frame rate by themselves,
                according to the timing requirements.
...
sensorInfo_t gc2145Info =
    {
...
#if defined(CONFIG_FIBOCOM_BASE) && (defined(_FIBOCOM_EXAMPLE_CAM_GC2145_1600X1200_RAW10) || defined(_FIBOCOM_EXAMPLE_CAM_GC2145_1600X1200_YUV422))
        1600,              //		uint16_t spi_pixels_per_line;	// max width of source image
        1200,              //		uint16_t spi_pixels_per_column; // max height of source image
#else // UNISOC
        ...
        1600,              //		uint16_t dstWinColEnd;
        0,                //		uint16_t dstWinRowStart;
        1200,              //		uint16_t dstWinRowEnd;
#else // UNISOC
        0,                //		uint16_t dstWinColStart;
...
#if defined(CONFIG_FIBOCOM_BASE) && defined(_FIBOCOM_EXAMPLE_CAM_GC2145_1600X1200_RAW10)
        CAM_FORMAT_RAW10,        //		cameraImageFormat_t image_format; // define in SENSOR_IMAGE_FORMAT_E enum,
#else // UNISOC
