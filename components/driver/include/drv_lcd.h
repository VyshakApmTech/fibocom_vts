...
#define DRV_LCD_ERR_MALLOC              (-12)   /* malloc error */
#define DRV_LCD_ERR_DEV_EMTPY           (-13)   /* device empty */
#define DRV_LCD_ERR_OPERATION           (-14)   /* operation failed */
#define DRV_LCD_ERR_IN_SLEEP            (-15)   /* device in sleep */
#define DRV_LCD_ERR_NOT_SUPPORT         (-16)   /* operation not support */

#if defined(CONFIG_DOT_MATRIX_LCD_SUPPORT)
#define DOT_LCD_ID_UNSUPPORT            (0X55)  // un-support read ID case
#endif

#define _LCD_DEV_NAME_LEN               (20)


/**
 * @brief Struction of multi LCD context
 */
typedef struct
{
    bool is_used;       // multi lcd function is used or not
    uint32_t open_num;  // lcd device opened number
} multi_lcd_ctx_t;

/**
 * @brief Struction of multi LCD configuration
 */
typedef struct
{
    uint16_t csx_pin;   /* LCD_CSX pin number, used with its GPIO function. */
    uint8_t csx_pol;    /* LCD_CSX pin polarity (the idle level). */
    uint16_t rst_pin;   /* LCD_RESET pin number, used with its GPIO function. */
    uint8_t rst_pol;    /* LCD_RESET pin polarity (the idle level). */
#if 0 // TODO: for future implementation
    uint16_t te_pin;    /* LCD_TE pin number, used with its GPIO function. */
    uint8_t te_pol;     /* LCD_TE pin polarity (the idle level). */
#endif
} multi_lcd_cfg_t;
#endif
