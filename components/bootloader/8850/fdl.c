...
{
    unsigned max_size, start_addr = d->dnld.start_address;

    if (d->secure_boot_enable)
    {
        switch (start_addr)
        {
        case CONFIG_SPL_FLASH_ADDRESS:
            max_size = CONFIG_APP_FLASH_SIZE;
            break;
#ifndef CONFIG_BOOT_FLASH_DISABLE
        case CONFIG_BOOT_FLASH_ADDRESS:
            max_size = CONFIG_BOOT_FLASH_SIZE;
            break;
#endif
        case CONFIG_APP_FLASH_ADDRESS:
            max_size = CONFIG_APP_FLASH_SIZE;
            break;
#ifdef CONFIG_APPIMG_FLASH_ADDRESS // some targets not support appimg
        case CONFIG_APPIMG_FLASH_ADDRESS:
            max_size = CONFIG_APPIMG_FLASH_SIZE;
            break;
#endif
        default:
            return -BSL_REP_DOWN_DEST_ERROR;
        ...
        if (!simageValid((const simageHeader_t *)(d->flash_mem), max_size))
        {
            return -BSL_ERROR_CHECKSUM;
        }
...
static void prvPowerOffPolling(fdlContext_t *d)
{
    ...
    if (BOOT_DNLD_FROM_USERIAL(d->device_type))
    {
        ...
        //in boot,it can't have usb connected driver,so use v-charge status
        if (!d->channel->connected(d->channel))
       #else
        unsigned val = 0;
        val = halAdiBusRead(&hwp_pmicAna->chgr_status);
        if ((val & 0x8) == 0)
       #endif
        {
...
void bootStart(uint32_t param)
{
    ...
    halAdiBusWrite(&hwp_pmicPsm->reserved_2, 0);
    // disable PSM module
