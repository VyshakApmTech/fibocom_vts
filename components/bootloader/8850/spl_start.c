...
#ifdef CONFIG_FIBOCOM_BASE
    extern void fibo_display_fw_ota_logo(int block_count, int cur_block);
    fibo_display_fw_ota_logo(block_count, block);
#endif
}

#define OTA_V2_PATH_MAX 256
void ota_v2_fw_global(char *dir)
{
    char tmp_path[OTA_V2_PATH_MAX] = {0};

    strcat(tmp_path, dir);
    strcat(tmp_path, "/");
    strcat(tmp_path, CONFIG_FUPDATE_PACK_FILE_NAME);
    gFupdatePackFileName = strdup(tmp_path);

    memset(tmp_path, 0, OTA_V2_PATH_MAX);
    strcat(tmp_path, dir);
    strcat(tmp_path, "/");
    strcat(tmp_path, CONFIG_FUPDATE_STAGE_FILE_NAME);
    gFupdateStageFileName = strdup(tmp_path);

    memset(tmp_path, 0, OTA_V2_PATH_MAX);
    strcat(tmp_path, dir);
    strcat(tmp_path, "/");
    strcat(tmp_path, CONFIG_FUPDATE_TEMP_FILE_NAME);
    gFupdateTempFileName = strdup(tmp_path);

    memset(tmp_path, 0, OTA_V2_PATH_MAX);
    strcat(tmp_path, dir);
    strcat(tmp_path, "/");
    strcat(tmp_path, CONFIG_FUPDATE_SIGN_FILE_NAME);
    gFupdateSignFileName = strdup(tmp_path);
...
{
    int ota_v2_dir_fd = -1;  //This file saved upgrade directory
    int fota_appfw_flag_fd = -1;
    char ota_v2_dir[OTA_V2_PATH_MAX] = {0};
    char ota_v2_delta_app_path[OTA_V2_PATH_MAX] = {0};

    fsMountSetScenario(FS_SCENRARIO_BOOTLOADER);
    if (!fsMountAll())
        return FUPDATE_RESULT_NOT_READY;

    ota_v2_dir_fd = vfs_open(FIBO_OTA_V2_DIR_SAVED_FILE, O_RDONLY);
    if(-1 != ota_v2_dir_fd){
        if(vfs_file_size(FIBO_OTA_V2_DIR_SAVED_FILE) != vfs_read(ota_v2_dir_fd , ota_v2_dir, vfs_file_size(FIBO_OTA_V2_DIR_SAVED_FILE))){
                OSI_LOGI(0,"[OTA_V2]read ota_v2_dir fail");
                return FUPDATE_RESULT_NOT_READY;
        }
        vfs_close(ota_v2_dir_fd);
        ota_v2_fw_global(ota_v2_dir);
    }

    //If da_df_upgrade flag exists, it means that power off when split df_da_merge.pack, won't trigger upgrade.
    fota_appfw_flag_fd = vfs_open(FILE_FOTA_APPFW_FLAG, O_RDONLY);
    if(-1 != fota_appfw_flag_fd)
    {
        vfs_close(fota_appfw_flag_fd);
        if(vfs_file_size(FIBO_OTA_V2_DIR_SAVED_FILE) < 0) //Enter ota V1
        {
        ...
        {
            strcat(ota_v2_delta_app_path, ota_v2_dir);
            strcat(ota_v2_delta_app_path, "/");
            strcat(ota_v2_delta_app_path, FIBO_OTA_V2_DELTA_APP);//get delta_app.pack path
            vfs_unlink(ota_v2_delta_app_path);
            vfs_unlink(gFupdatePackFileName);
            vfs_unlink(gFupdateSignFileName);
            vfs_rmdir(ota_v2_dir);
        }
