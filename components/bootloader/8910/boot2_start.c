...
#ifdef CONFIG_FIBOCOM_FW_UPDATE_RESULT
#define UPDATE_RESULT "/update_result"
#define FW_UPDATE_RESULT UPDATE_RESULT "/" "fw_update_result"
#endif
...
static void prvFirmwareUpdate(void)
{
    ...
    if (fupdateGetStatus() == FUPDATE_STATUS_READY)
    {
        ...
        if (!fupdateIsVersionUpgrade())
        {
            OSI_LOGE(0x10009f30, "FUPDATE new version upgrade: 0");
            fsUmountAll();
            return;
        }
#endif
...
#ifdef CONFIG_FIBOCOM_FW_UPDATE_RESULT 
    vfs_mkdir(UPDATE_RESULT, 0);
    vfs_file_write(FW_UPDATE_RESULT, &result, sizeof(result));
#endif
...
static void prvTimerCallback(void *param)
{
...
void bootStart(uint32_t param)
{
...
#ifdef CONFIG_BOOT_TIMER_IRQ_ENABLE
    bootEnableInterrupt();
    bootEnableTimer(prvTimerCallback, NULL, CONFIG_BOOT_TIMER_PERIOD);
#endif
    ...
    if (ch == NULL)
    {
        OSI_LOGE(0x1000907f, "BOOT fail, can't open uart fdl channel");
        osiPanic();
    }
    ...
    if (fdl == NULL)
    {
        OSI_LOGE(0x10009080, "BOOT fail, can not create fdl engine");
        osiPanic();
    }
    ...
    for (;;)
    {
        ...
        if (fdlEngineReadRaw(fdl, &sync, 1))
        {
            ...
            OSI_LOGI(0x10009081, "BOOT recv 0x%x , recv 0x7e cnt %d", sync, sync_cnt);
        }
        if (sync_cnt >= 3)
        {
            fdlEngineSendVersion(fdl);

            OSI_LOGI(0x10009082, "ENTER DOWNLOAD MODE ");
            osiDelayUS(50 * 1000);
