...
static void prvSetBaud(fdlEngine_t *fdl, fdlPacket_t *pkt)
{
    ...
    OSI_LOGD(0x10009085, "FDL1: change baud %d", baud);
...
static void prvDataStart(fdlEngine_t *fdl, fdlPacket_t *pkt, fdlDataDnld_t *dnld)
{
    ...
    OSI_LOGI(0x10009086, "FDL1: start_addr 0x%x, size %d ......", start_addr, file_size);
    if (CONFIG_FDL2_IMAGE_START == start_addr)
    {
        if (file_size > CONFIG_FDL2_IMAGE_SIZE)
        {
            OSI_LOGE(0x10009087, "fdl2 size beyond the limit...");
    ...
    {
        OSI_LOGE(0x10009088, "fdl2 addr error...");
...
static void prvDataMidst(fdlEngine_t *fdl, fdlPacket_t *pkt, fdlDataDnld_t *dnld)
{
    ...
    if ((dnld->stage != SYS_STAGE_START) && (dnld->stage != SYS_STAGE_GATHER))
    {
        ...
        OSI_LOGE(0x10009089, "FDL1: down start err...");
        osiPanic();
    }
    ...
    {
        fdlEngineSendRespNoData(fdl, BSL_REP_DOWN_SIZE_ERROR);
        OSI_LOGE(0x1000908a, "FDL1: down size err...");
        osiPanic();
    }

    OSI_LOGV(0x1000908b, "write addr %x, size %d", dnld->write_address, data_len);
...
static void prvDataEnd(fdlEngine_t *fdl, fdlDataDnld_t *dnld)
{
    ...
    {
        //recv all packets
        OSI_LOGD(0x1000908c, "FDL1: write file ok...");
        fdlEngineSendRespNoData(fdl, BSL_REP_ACK);
        dnld->stage = SYS_STAGE_END;
    }
    else
    {
        OSI_LOGD(0x1000908d, "FDL1: secure boot verify fail...");
...
void bootStart(uint32_t param)
{
    ...
    if (ch == NULL)
    {
        OSI_LOGE(0x10009091, "FDL1 failed to open device");
        osiPanic();
    }
    ...
    if (fdl == NULL)
    {
        OSI_LOGE(0x10009092, "FDL1 failed to create engine");
        osiPanic();
    }
    ...
    for (;;)
    {
        ...
        if (fdlEngineReadRaw(fdl, &sync, 1) > 0 && sync == HDLC_FLAG)
        {
            ...
            {
                OSI_LOGE(0x10009093, "FDL fail to send version string");
                osiPanic();
            }
            break;
        }
