...
typedef enum
{
    BLE_CLIENT_START_MSG = 0x5001,
    BLE_CLIENT_START_DISCOVER_MSG,

    BLE_SERVER_START_MSG = 0x5020,
    BLE_SERVER_SMP_REQ_MSG,

    BT_PHONE_START_MSG = 0x5030,
    BT_PHONE_CONNECT_A2DP_MSG,
    BT_PHONE_AVRCP_POSITION_MSG,
    BT_PHONE_HFP_DIAL,
    BT_PHONE_OPEN_MSG,

    BT_HEADSET_START_MSG = 0x5040,
    BT_SSP_DISPLAY_RESULT_MSG,

    BT_SPP_START_MSG = 0x5050,
    BT_SPP_STOP_MSG,
    BT_SPP_RESEND_MSG,
} bt_demo_task_msg_t;
...
typedef enum
{
    ...
    BT_OPP_START_MSG,
    BT_OPP_CON_MSG, //0xA504
    BT_OPP_PUT_MSG,
    BT_FTP_START_MSG, //0xA506
    BT_FTP_CON_MSG,
    BT_FTP_BROWSING_MSG, //0xA508
    BT_FTP_PULL_MSG,
    BT_FTP_PUSH_MSG, //0xA50A
    BT_FTP_END_MSG,
    BT_RECONNECT_ACL_MSG,
    BT_RECONNECT_PROFILE_MSG,
} BT_MSG_E;

typedef struct bt_msg_tag
{
    uint16 msg_id;
    uint16 reserved;
    void *msg_data;
...
void app_connect_a2dp_src(void);
void app_bt_send_position_resp(void *pArgs);
void ble_server_demo_start_cb(void *argv);
void ble_client_demo_start_cb(void *argv);
uint32_t BT_Debug_cmd_opcode(void);

void bt_app_headset_task_create(void);
