...
#define FIBO_SMS_MESSAGE_LENGTH_MAX 380 //2*(180 + 12) - 4
...
#define FIBO_SMS_DEL_ALL_STORAGE       0xFFFFFFFF

#define FIBO_SMS_STORAGETYPE                "SMSSTORAGETYPE"
#define FIBO_READ_TEXT_SMS_FORMAT           "SMSCSCSTYPE"
#define FIBO_TEL_NUMBER_MAX_LEN 21
...
{
    uint16_t nIndex;
    uint8_t sca[FIBO_TEL_NUMBER_MAX_LEN];
    uint8_t sca_size;
    uint8_t tosca;
    uint8_t fo;
    uint8_t pid;
...
typedef enum
{
    ...
    FIBO_SMS_NEW_MSG_IND,    //Text Message
    FIBO_SMS_NEW_PDU_MSG_IND,    //PDU Message
    FIBO_SMS_LIST_IND,
...
typedef enum
{
    E_FIBO_SMS_MESSAGE_MODE_PDU       = -2,   /**<  Message type PDU */
...
typedef struct
{
    ...
    uint8_t free_slots_valid;//兼容前面版本
    uint32_t free_slots;//兼容前面版本
} fibo_sms_store_max_size_info_t;

void fibo_sms_register_cb(uint32_t event_id, void* cb_data);
...
/*********************************************************************************
  *Function:  fibo_sms_send_smspdu
  *Description: send PDU sms data to other
  *Input:h_sms              : handle of client
         raw_message_data   : PDU data
         raw_resp           : result of send PDU
         
  *Output:
  *Return:  0 -> success other ->fail
  *Others:
**********************************************************************************/
int fibo_sms_send_smspdu( sms_client_handle_type      h_sms,
                         fibo_sms_send_raw_message_data_t raw_message_data,
                         fibo_sms_raw_send_resp_t *raw_resp);
...
/*********************************************************************************
*Function:  fibo_sms_getmsglist
*Description: get list about sms 
*Input:h_sms              : handle of client
      req_info            : mode about read sms list
      resp_info           : result of sms list
      
*Output:
*Return:  0 -> success other ->fail
*Others:
**********************************************************************************/
int fibo_sms_getmsglist(sms_client_handle_type h_sms,     fibo_sms_msg_list_req_info_t *req_info,    fibo_sms_msg_list_info_t *resp_info);

/*********************************************************************************
*Function:  fibo_sms_getmaxstoresize
*Description: get storage info about sms 
*Input:h_sms              : handle of client
      pt_sms_storage      : storage type
      pt_info           : result of sms storage info
      
*Output:
*Return:  0 -> success other ->fail
*Others:
**********************************************************************************/
int fibo_sms_getmaxstoresize(sms_client_handle_type h_sms, fibo_sms_get_store_max_size_info_t *pt_sms_storage, fibo_sms_store_max_size_info_t *pt_info);

int32 fibo_sms_set_config(uint8 simId, char *name, void *input_param, void *output_param);


int32 fibo_sms_get_config(uint8 simId, char *name, void *input_param, void *output_param);
