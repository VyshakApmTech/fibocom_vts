...
/**
 * @brief Physical layer API: read uart data.
 * 
 * @param uart_port uart port number
 * @param buf Buffer for storing received data 
 * @param len Maximum length of received data
 * @param timeout Receiving timeout in milliseconds
 * @return INT32 -1: failed
 *               >= 0: length of received data
 * 
 * @note It can be called to receive data from uart in polling mode, when parameter 'recv_cb' of 
 *       fibo_hal_uart_init() is NULL.
 */
INT32 fibo_pi_read_uartdata(hal_uart_port_t uart_port, UINT8 *buf, UINT32 len, UINT32 timeout);
