...
#if defined(CONFIG_FFW_EC200_MQTT_AT_ENABLE)
/// del will
#define ffw_mqttc_i_del_will(c) ((c)->del_will)(c)

/// connect mqtt in async mode， QMQTT
#define ffw_mqttc_i_connect3(c, host, port, clearsession, keepalive, tls, timeout) ((c)->connect3)(c, host, port, clearsession, keepalive, tls, timeout)

// mqtt disconnect,QMQTT 
#define ffw_mqttc_i_disconnect1(c) ((c)->disconnect1)(c)
#endif
/// connect
...
#if defined(CONFIG_FFW_EC200_MQTT_AT_ENABLE)
    // MQTT TCP ACK
    FFW_MQTT_TCP_CONN_ACK,

    // MQTT TCP DISCONN
    FFW_ONLY_MQTT_DISCONN,

    FFW_TCP_MQTT_DISCONN,
#endif
    /// TOTAL COUNT
    FFW_MQTT_EVT_COUNT,
};
...
#if defined(CONFIG_FFW_EC200_MQTT_AT_ENABLE)
    /// 消息回调函数，msgid
    ffw_mqttc_interface_on_data_msgid_t data_msgid_cb;
#endif
    /// 回调上下文
    void *arg;
};
...
#if defined(CONFIG_FFW_EC200_MQTT_AT_ENABLE)
    int (*del_will)(ffw_mqttc_interface_t *c);

    /// 异步连接函数,QMTOPEN
    int (*connect3)(ffw_mqttc_interface_t *c, const char *host, uint16_t port, bool clearsession, uint32_t keepalive, bool tls, int32_t timeout);

    int (*sub1)(ffw_mqttc_interface_t *c, int msgid, const char *topic, int topic_len, int qos);

    /// 断开连接
    int (*disconnect1)(ffw_mqttc_interface_t *c);
#endif
    /// 是否已经连接
