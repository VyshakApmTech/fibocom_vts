 ...
 * @return false 不相等
 */
bool ffw_str_equal1(const char *str1, const char *str2, int n, bool case_sensetive);
...
void ffw_compress_sequence(uint32_t arr[], uint32_t n, char *arry_out);
...
* @param len
 */
void ffw_bubble_sort(uint32_t arr[], int len);

void *ffw_realloc(void *p, size_t size);

bool ffw_is_big_endian();
