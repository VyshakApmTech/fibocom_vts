...
extern int32_t cbb_cmd_gtrejcause_read(reject_code_t *rejcause, uint8_t *set_cfg);

extern int32_t cbb_qcsq_set(uint8_t enable);
extern int32_t cbb_qcsq_read(uint8_t *enable);
extern int32_t cbb_cmd_qcsq_sysmode_fomat(char *sys_mode);
extern int32_t cbb_cmd_qcsq_exe(char *rsp);
extern void fibo_qcsq_urc(uint8_t simId);
