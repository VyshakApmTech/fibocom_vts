...
#define CBB_HEARTBEAT_RECONNECT_FOREVER 0xFFFFFFFF
...
#define CBB_HEARTBEAT_BUFFER_MAX 1460
...
typedef struct
{
    ...
    /// tls min version
    int32_t ssl_min_version;

    /// tls max version
    int32_t ssl_max_version;
...
int32_t cbb_cmd_heartbeat_reconnect_time(uint8_t id, uint32_t time);
...
int32_t cbb_cmd_heartbeat_data_set(uint8_t id, const char *data, int32_t len);
int32_t cbb_cmd_heartbeat_interval_set(uint8_t id, uint32_t interval);
int32_t cbb_cmd_heartbeat_tls_ver_set(uint8_t id, int32_t min, int32_t max);
int32_t cbb_cmd_heartbeat_reconnect_interval(uint8_t id, int32_t timeout);
#endif
