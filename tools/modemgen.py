...
def prepack_cfg_from_json(cfg, srctop, bintop):
    ...
    srctop = srctop.replace('\\', '/')
    bintop = bintop.replace('\\', '/')
...
# get sffs image unused space enough or not
# return : 0--ok other--not enough
def sffsimg_freespace_check(filelist_json,image_file):
    comm = 'dtools fbdevgen -d v '+filelist_json+' '+image_file
    content,result = exec_cmd(comm)
    # result = out.returncode
    if result != 0:
        print(comm)
        print(content)
        print("generate modem size check image failed")
        return -1

    # check unused block count
    lines = re.split(r'\n', content.decode())
    #print(lines)
    count=len(lines)
    # find lzmar block size and PB size
    if count >= 3:
        data = lines[0].strip().split()  #PB size
        #print(data)
        PB_size = int(data[8].replace(",",""),16)
        #print(PB_size)
        block_size = PB_size - 12
        data = lines[1].replace("=",",").strip().split(",")  #lzmar block size
        #print(data)
        lzmar_block_size = int(data[1])
        #print(lzmar_block_size)
        data = lines[count-2].replace("/"," ").strip().split() #sffs block used used/total
        #print(data)
        used_block_count = int(data[3])
        total_block_count = int(data[4])
        free_block_count = total_block_count - used_block_count
        free_size = free_block_count * block_size
        print("sffs image free size/min size", free_size,"/",lzmar_block_size)
        if lzmar_block_size == 65536:
            lzmar_block_size -= 11536
        #free blocks total size should > lzmar block size
        if free_size >= lzmar_block_size:
            return 0
        else:
            print(comm)
            print("free size < min size, create image failed!")
            return -1
...
def mimggen8910(args):
    ...
    # Run modem partition available size check
    modemcheckimg = os.path.join(workdir, 'modem_size.img')
    if sffsimg_freespace_check(mgenfile, modemcheckimg) != 0:
        print(args.variant, "/modem partition free space is not enough!")
        return -1
...
def mvargen8850_args(sub_parser):
    ...
    parser.add_argument('--cpbin', dest='cpbin', default=None,
                        help='input the cp.bin file name')
...
def mvargen8850(args):
    ...
    if(args.apbin):
        ...
        # add ap files to modem image json
        for item in apbingen:
            filename = item["file"].replace("modem/","")
            item["file"] = filename
            filelist["plain_file"].append(item)
    ...
    # add cp files to modem image json
    for item in cpgen:
        filename = item["file"].replace("modem/","")
        item["file"] = filename
        filelist["plain_file"].append(item)

    filelist['lzma_block_size'] = lzma_block_size
    filelist['dict-size'] = lzma_dict_size
    mgen['partiton'].append(filelist)

    #generate modem files list json
    mgenfile = os.path.join(workdir, 'modem.json')
    with open(mgenfile, 'w') as fh:
        json.dump(mgen, fh, indent=4)
