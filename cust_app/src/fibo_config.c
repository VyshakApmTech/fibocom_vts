...
#ifdef CONFIG_FFW_ZIP_ENABLE
int ffw_file_unzip(const char *zipfilename, const char *dst_dir, int without_path);
#endif
...
uint8_t custom_audio_levelplay = 0;
...
uint8_t flight_mode_set_from_cus = 1;
...
fibo_config_t *fibo_get_config(void)
{
    ...
    */
    ...
    fibo_config.unzip_without_dir = 1;

    fibo_config.cam_rst_pin_unused = false;
    //Send AT commands using UART1
    fibo_config.uart1TxAtCmd = false;

    ///MTC0671-184 sundaqing 20250707 start
    fibo_config.http_post_no_copy = false;
...
int fibo_get_unzip_option(void)
{
    int ret = 0;
    fibo_config_t *config = fibo_get_config();
    ret = config->unzip_without_dir;
    return ret;
}

//2025-04-16, mahongmei, MTC0626-50, Begin. Open-source fibo_file_unzip API
INT32 fibo_file_unzip(const char *file_path, const char *zip_path)
{
#ifdef CONFIG_FFW_ZIP_ENABLE
    int opt_extract_without_path = 1;
    opt_extract_without_path = fibo_get_unzip_option(); 
    OSI_PRINTFE("opt_extract_without_path=%d", opt_extract_without_path);
    return ffw_file_unzip(file_path, zip_path, opt_extract_without_path);
#else
    OSI_PRINTFE("%s is not supported.", __FUNCTION__);
    return -1;
#endif
}
//2025-04-16, mahongmei, MTC0626-50, End.
